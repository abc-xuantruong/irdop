...
const Header = () => {
    ...
    const displayName = currentUser?.identity_name ? (currentUser.identity_name.length > 60 ? currentUser.identity_name.substring(0, 60) + "..." : currentUser.identity_name) : "Tài khoản";
            ...
            <div className="flex justify-between items-center w-full px-5 ">
                ...
                <div className="md:flex items-center hidden">
                    {currentPath.includes("/intra-h1y25-c1") ? (
                        ...
                        <>
                            <a href="https://chat.irdop.org/" target="_blank" rel="noopener noreferrer" className="cursor-pointer md:text-md ml-4 text-md font-medium text-teritary hover:text-primary">
                                Chat
                            </a>
                            <Link
                                to={getLinkWithFilter("/processing")}
                                className={`cursor-pointer md:text-md ml-4 text-md font-medium ${currentPath.includes("/processing") ? "text-primary" : "text-teritary hover:text-primary"}`}
                            >
                                Lab
                            </Link>
                            {shouldShowReception() && (
                            ...
                            <Link
                                to="/progress"
                                className={`cursor-pointer md:text-md ml-4 text-md font-medium ${currentPath.includes("/progress") ? "text-primary" : "text-teritary hover:text-primary"}`}
                            >
                                Tiến trình
                            </Link>
                            <Link
                                to="/library"
                                className={`cursor-pointer md:text-md ml-4 text-md font-medium ${currentPath.includes("/library") ? "text-primary" : "text-teritary hover:text-primary"}`}
                            >
                                Thư viện
                            </Link>
                ...
                <div className="md:hidden relative flex items-center justify-end" ref={dropdownRef}>
                    {currentPath.includes("/login") ? (
                        ...
                        <>
                            <div className="relative flex items-center">
                                {" "}
                                <p className="text-primary cursor-pointer text-end text-lg font-semibold" onClick={() => setDropdownOpen(!dropdownOpen)}>
                                    {displayName}
                                </p>
                            </div>
                            <div
                            ...
                            >
                                <div className="flex flex-col w-full h-full">
                                    <div className="p-2 pl-3 border-b-2 bg-gray-50 flex justify-between items-center">
                                        <p className="text-base font-medium truncate w-full text-start cursor-pointer" onClick={() => handleNavigate("/user-dashboard")}>
                                            {displayName}
                                        </p>
                                        <button onClick={() => setDropdownOpen(false)} className="text-gray-500 hover:text-gray-700 p-2"></button>
                                    </div>
                            ...
                            {dropdownOpen && <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setDropdownOpen(false)}></div>}
                        </>
                    )}
                </div>
            </div>
