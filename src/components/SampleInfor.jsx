...
const SampleInfor = () => {
    ...
    const [technicianDropdownVisible, setTechnicianDropdownVisible] =
        useState(null);
    const [dropdownPosition, setDropdownPosition] = useState({
        top: 0,
        left: 0,
    });
    const [deadlineDropdownVisible, setDeadlineDropdownVisible] =
        useState(null);
    ...
    const [isTransferMultipleVisible, setIsTransferMultipleVisible] =
        useState(false);
    ...
    const [isParameterWarningVisible, setIsParameterWarningVisible] =
        useState(false); // Add state for parameter warning modal
    ...
    useEffect(() => {
        const checkOverflow = () => {
            if (statusContainerRef.current) {
                const isOverflowing =
                    statusContainerRef.current.scrollWidth >
                    statusContainerRef.current.clientWidth;
        ...
        try {
            ...
            if (field === "sample_name") {
                ...
                const updatedCustomerInfo = customerInfo.map((item) => {
                    if (
                        item.fname.includes("Tên mẫu thử") ||
                        item.fname.includes("name")
                    ) {
                        return { ...item, fvalue: value };
                    }
                    return item;
                });
                updatedSample.sample_information = [
                    ...updatedCustomerInfo,
                    ...receiptInfo,
                ];
            ...
            const response = await apiPost(
                "https://black.irdop.org/to82oe92i/db/update/sample",
                {
            ...
            if (response.status === 200) {
                ...
                Swal.fire({
                    ...
                    title: "Lỗi",
                    text:
                        response.data?.message || `Failed to update ${field}.`,
                });
            }
        } catch (error) {
    ...
    const openPPTWindow = () => {
        ...
        if (sample?.report?.length > 0) {
            ...
            const latestReport = sample.report.reduce((prev, current) =>
                new Date(prev.publish_date) > new Date(current.publish_date)
                    ? prev
                    : current,
            );
            ...
            window.open(
                `${window.location.origin}/report?sample_uid=${sample_uid}`,
                "_blank",
            );
        }
    };
    ...
    const fetchReceiptFull = async () => {
        try {
            const response = await apiGet(
                `https://black.irdop.org/khsi19me/db/get/receipt_full/${receipt_uid}`,
            );
    ...
    const handleSampleSelectFromDropdown = async (sampleUid) => {
        ...
        let analyses = receiptFull.samples.find(
            (sample) => sample.sample_uid === sampleUid,
        ).analysis;
        ...
        analyses = analyses.map((analysis) => {
            ...
            const { id, result_value, reviewed_by, ...cleanAnalysis } =
                analysis;
        ...
        const existingParameterNames = listAnalytes.map((a) =>
            a.parameter_name.toLowerCase().trim(),
        );
        const filteredAnalyses = analyses.filter(
            (analysis) =>
                !existingParameterNames.includes(
                    analysis.parameter_name.toLowerCase().trim(),
                ),
        );

        console.log(
            `Filtered out ${
                analyses.length - filteredAnalyses.length
    ...
    const searchParameters = async (query) => {
        try {
            const response = await apiPost(
                "https://black.irdop.org/ha8i0uw2/db/search/parameter",
                {
                    query,
    ...
    const handleConfirmAddParameter = async () => {
        try {
            ...
            const filteredParameters = selectedParameters.filter(
                (param) =>
                    !existingParameterNames.includes(
                        param.parameter_name.toLowerCase().trim(),
                    ),
            );

            if (filteredParameters.length === 0) {
                showToast(
                    "Tất cả chỉ tiêu đã tồn tại trong mẫu này",
                    "warning",
                );
            ...
            const response = await apiPost(
                "https://black.irdop.org/trelw82ki/db/insert/bulk/analysis",
                {
                    analyses: parameters,
                },
            );
            ...
            if (response.status === 200) {
                showToast(
                    `${response.data.length} chỉ tiêu được thêm thành công!`,
                );
                setIsAddingParameter(false);
                setSelectedParameters([]);
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "An error occurred while adding parameters.",
            });
        }
    };
    ...
    const updateParameterAndAnalysis = async (analysis) => {
        try {
            ...
            if (
                ...
                const parameterResponse = await apiPost(
                    "https://black.irdop.org/ha8i0uw2/db/upsert/parameter",
                    {
                        parameter: {
                ...
                if (parameterResponse.status === 200) {
                    ...
                    if (analysis.protocol_code)
                        updateData.protocol_code = analysis.protocol_code;
                    if (analysis.protocol_source)
                        updateData.protocol_source = analysis.protocol_source;

                    // Now update the analysis with the new parameter_id
                    const analysisResponse = await apiPost(
                        "https://black.irdop.org/trelw82ki/db/update/analysis",
                        {
                            analysis: updateData,
                        },
                    );
                    ...
                    Swal.fire({
                        ...
                        title: "Lỗi",
                        text:
                            parameterResponse.data?.message ||
                            "Lỗi khi cập nhật thư viện chỉ tiêu.",
                    });
                    return analysis;
                }
            } else {
                ...
                if (analysis.parameter_name !== undefined)
                    changedFields.parameter_name = analysis.parameter_name;
                if (analysis.result_value !== undefined)
                    changedFields.result_value = analysis.result_value;
                if (analysis.result_unit !== undefined)
                    changedFields.result_unit = analysis.result_unit;
                if (analysis.protocol_code !== undefined)
                    changedFields.protocol_code = analysis.protocol_code;
                if (analysis.protocol_source !== undefined)
                    changedFields.protocol_source = analysis.protocol_source;
                if (analysis.deadline !== undefined)
                    changedFields.deadline = analysis.deadline;
    ...
    const onUpdateAnalysis = async (analysis) => {
        try {
            ...
            if (
                ...
                if (analysis.parameter_name !== undefined)
                    fieldBeingUpdated.parameter_name = analysis.parameter_name;
                if (analysis.result_value !== undefined)
                    fieldBeingUpdated.result_value = analysis.result_value;
                if (analysis.result_unit !== undefined)
                    fieldBeingUpdated.result_unit = analysis.result_unit;
                if (analysis.protocol_code !== undefined)
                    fieldBeingUpdated.protocol_code = analysis.protocol_code;
                if (analysis.protocol_source !== undefined)
                    fieldBeingUpdated.protocol_source =
                        analysis.protocol_source;
                if (analysis.technician_uid !== undefined)
                    fieldBeingUpdated.technician_uid = analysis.technician_uid;
                if (analysis.deadline !== undefined)
                    fieldBeingUpdated.deadline = analysis.deadline;
    ...
    const handleSaveNewParameter = async () => {
        try {
            ...
            if (updateParameterMode) {
                ...
                if (parameterResponse.status !== 200) {
                    Swal.fire({
                        ...
                        text:
                            parameterResponse.data?.message ||
                            "Lỗi khi tạo chỉ tiêu trong thư viện.",
                    });
                    return;
                }
            ...
            const response = await apiPost(
                "https://black.irdop.org/trelw82ki/db/insert/analysis",
                {
    ...
    const renderNewParameter = () => {
        const paginatedParameters = parameterList.slice(
            (currentPage - 1) * 5,
            currentPage * 5,
        );
                    ...
                    <div className="w-full h-full relative flex flex-col justify-between overflow-auto">
                        ...
                        <div className="mb-4 h-full flex overflow-y-auto text-sm flex-wrap content-start">
                            {selectedParameters.map((parameter, index) => (
                                ...
                                >
                                    <div>
                                        <p className="text-xs font-medium w-full line-clamp-1">
                                            Nền mẫu: {parameter.matrix}
                                        </p>
                                        <p className="text-primary font-medium w-full line-clamp-1">
                                            {parameter.parameter_name}
                                        </p>
                                        <p className="text-start text-text-secondary w-full line-clamp-1">
                                            {parameter.protocol_code}
                                    ...
                                    <button
                                        className="text-red-500 px-2 py-3 ml-1"
                                        onClick={() =>
                        ...
                        <div className="flex justify-between items-center">
                            <div className="relative flex">
                                ...
                                <button
                                    className="bg-white border-gray-300 p-2 rounded ml-2"
                                ...
                                {isDropdownVisible && (
                                    <div className="absolute bg-white border rounded w-full max-h-72 overflow-y-auto mt-2 z-10 bottom-10">
                                        <ul>
                                            {receiptFull.samples.map(
                                                (sample) => (
                                                    <li
                                                        key={sample.sample_uid}
                                                        className="p-2 border-b cursor-pointer hover:bg-gray-200"
                                                        onClick={() =>
                                                            handleSampleSelectFromDropdown(
                                                                sample.sample_uid,
                                                            )
                                                        }
                                                    >
    ...
    useEffect(() => {
        const fetchSample = async () => {
            try {
                const response = await apiGet(
                    `https://black.irdop.org/to82oe92i/db/get/sample_full/${sample_uid}`,
                );
                ...
                if (response.data && response.data.analysis) {
                    for (const analysis of response.data.analysis) {
                        if (analysis.reviewed_by) {
                            ...
                            const reviewerData = await getIdenByUid(
                                analysis.reviewed_by,
                            );
                            analysis.reviewerName = reviewerData
                                ? reviewerData.identity_name
                                : "Unknown";
                        }
                    }
                }
                ...
                if (
                    response.data.sample_information &&
                    response.data.sample_information.length > 0
                ) {
    ...
    const handleSampleSelect = (sampleUid) => {
        navigate(
            `/dashboard/sample?receipt_uid=${receipt_uid}&sample_uid=${sampleUid}`,
        );
    };
    ...
    const handleSaveContent = async (newValue) => {
        ...
        try {
            const analysis = updatedAnalytes.find(
                (item) => item.id === parseInt(editingField.split("-")[2]),
            );
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "An error occurred while updating analysis.",
            });
        }
    };
    ...
    const handleConfirmEdit = async () => {
        try {
            ...
            Swal.fire({
                ...
                title: "Lỗi",
                text:
                    error.message || "An error occurred while updating sample.",
            });
        }
    };
    ...
    const handleCustomerFieldChange = (index, field, value) => {
        ...
        if (field === "fname") {
            const selectedField = defaultCustomerFields.find(
                (item) => item.fname === value,
            );
            if (selectedField) {
        ...
        if (
            field === "fvalue" &&
            (updatedCustomerInfo[index].fname.includes("Tên mẫu thử") ||
                updatedCustomerInfo[index].fname.includes("name"))
        ) {
    ...
    const handleReceiptFieldChange = (index, field, value) => {
        ...
        if (field === "fname") {
            const selectedField = defaultReceiptFields.find(
                (item) => item.fname === value,
            );
            if (selectedField) {
        ...
        if (
            field === "fvalue" &&
            (updatedReceiptInfo[index].fname.includes("Mô tả") ||
                updatedReceiptInfo[index].fname.includes("desc"))
        ) {
        ...
        if (
            field === "fvalue" &&
            (updatedReceiptInfo[index].fname.includes("Nền mẫu") ||
                updatedReceiptInfo[index].fname.includes("matrix"))
        ) {
    ...
    const handleSaveNewReport = async () => {
        ...
        if (receiptInfo.length > 0) {
            ...
            if (
                !firstReceiptField.fname.includes("Ngày tiếp nhận") &&
                !firstReceiptField.fname.includes("receipt date")
            ) {
                Swal.fire({
                    icon: "error",
                    title: "Lỗi",
    ...
    const handleCopySampleInfo = (sampleUid) => {
        ...
        const selectedSample = listSampleByReceipt.find(
            (s) => s.sample_uid === sampleUid,
        );
                ...
                <div className="flex flex-col md:flex-row md:overflow-auto">
                        ...
                        <div className="w-full overflow-hidden hover:overflow-auto md:pb-2 lg:pb-0 pb-0 hover:pb-0 mb-1">
                            ...
                            {customerInfo?.length === 0 && (
                                <div className="text-center text-gray-500 italic py-2">
                                    Chưa có thông tin khách hàng. Nhấn nút + để
                                    thêm thông tin.
                                </div>
                            )}
                        </div>
                    ...
                    <div className="w-full pt-2 md:pt-0">
                        ...
                        <div className="w-full overflow-hidden hover:overflow-auto md:pb-2 lg:pb-0 pb-0 hover:pb-0 mb-1">
                            ...
                            {receiptInfo?.length === 0 && (
                                <div className="text-center text-gray-500 italic py-2">
                                    Chưa có thông tin tiếp nhận. Nhấn nút + để
                                    thêm thông tin.
                                </div>
                            )}
                        </div>
                    ...
                    <div className="flex justify-between px-4 mt-4 pt-2 border-t">
                        <button
                            ...
                            onClick={() => {
                                ...
                                if (
                                    currentSample &&
                                    currentSample.sample_information
                                ) {
                                    const sampleInfo =
                                        currentSample.sample_information || [];
    ...
    const handleRemoveAnalyte = async (id) => {
        try {
            const response = await apiPost(
                "https://black.irdop.org/trelw82ki/db/delete/analysis",
                {
                    id,
                    modified_by_uid: currentUser.identity_uid,
                },
            ...
            if (response.status === 200) {
                ...
                setListAnalytes(
                    listAnalytes.filter((analyte) => analyte.id !== id),
                );
            } else {
                Swal.fire({
                    icon: "error",
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "An error occurred while deleting analysis.",
            });
        }
    };

    const technician = (param) => {
        const iden = technicians.find(
            (identity) => identity.identity_uid === param.technician_uid,
        );
    ...
    const toggleTechnicianDropdown = (index, event) => {
        ...
        setTechnicianDropdownVisible(
            technicianDropdownVisible === index ? null : index,
        );
    ...
    const handleTechnicianChange = async (index, identity_uid) => {
        ...
        try {
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "Có lỗi xảy ra khi cập nhật người thực hiện",
            });
        }
    };
    ...
    const toggleDeadlineDropdown = (index, event) => {
        ...
        setDeadlineDropdownVisible(
            deadlineDropdownVisible === index ? null : index,
        );
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                Swal.fire({
                    ...
                    text:
                        response.data?.message ||
                        "Có lỗi xảy ra khi cập nhật hạn trả",
                });
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        return () => {
            document.removeEventListener(
                "mousedown",
                handleClickOutsideParameterForm,
            );
        };
    }, [isAddingParameter]);
    ...
    const handleClickOutsideSampleDropdown = (event) => {
        if (
            !event.target.closest(".sample-dropdown-container") &&
    ...
    useEffect(() => {
        if (sampleDropdownVisible) {
            document.addEventListener(
                "mousedown",
                handleClickOutsideSampleDropdown,
            );
        } else {
            document.removeEventListener(
                "mousedown",
    ...
    const handleProtocolSourceChange = async (index, newValue) => {
        ...
        try {
            ...
            if (updateParameterMode) {
                const updatedAnalysis = await updateParameterAndAnalysis(
                    analysis,
                );
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "An error occurred while updating protocol source.",
            });
        }
    };
    ...
    const handleDeleteSampleConfirmAction = async () => {
        try {
            const response = await apiPost(
                "https://black.irdop.org/to82oe92i/db/delete/sample",
                {
                    id: deleteItemId,
                    modified_by_uid: currentUser.identity_uid,
                },
            ...
            Swal.fire({
                ...
                title: "Lỗi",
                text:
                    error.message || "An error occurred while deleting sample.",
            });
        } finally {
            setIsDeleteConfirmVisible(false);
            setDeleteItemId(null);
    ...
    const handleDeleteAnalysisConfirmAction = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                setListAnalytes(
                    listAnalytes.filter(
                        (analyte) => analyte.id !== deleteItemId,
                    ),
                );
            } else {
                Swal.fire({
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
            <div className="bg-white p-4 rounded-lg w-[400px] h-[200px] relative flex flex-col justify-between">
                ...
                <div className="flex justify-end mt-4">
                    <button
                        className="bg-gray-500 text-white p-2 rounded mr-2 w-1/4"
    ...
    const handleDeleteMultipleConfirmAction = async () => {
        try {
            ...
            if (response.status === 200) {
                showToast(
                    `${selectedAnalytes.length} analyses deleted successfully!`,
                );
                setListAnalytes(
                    listAnalytes.filter(
                        (analyte) => !selectedAnalytes.includes(analyte.id),
                    ),
                );
                setSelectedAnalytes([]);
                setSelectAll(false);
                ...
                Swal.fire({
                    ...
                    title: "Lỗi",
                    text:
                        response.data?.message || "Failed to delete analyses.",
                });
            }
        } catch (error) {
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "An error occurred while deleting analyses.",
            });
        } finally {
            setIsDeleteConfirmVisible(false);
        }
    ...
    const handleBulkTransferConfirm = async () => {
        ...
        try {
            ...
            const selectedItems = listAnalytes.filter((analyte) =>
                selectedAnalytes.includes(analyte.id),
            );
            ...
            for (const analyte of selectedItems) {
                try {
                    ...
                    console.error(
                        `Error updating analysis ID ${analyte.id}:`,
                        error,
                    );
                    failCount++;
                }
            ...
            if (failCount > 0) {
                ...
                showToast(
                    `Successfully transferred ${
                        ...
                        technicians.find(
                            (tech) => tech.identity_uid === selectedTechnician,
                        )?.identity_name
                    }`,
                );
            }
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "An error occurred while transferring analyses",
            });
        }
    };
    ...
    const renderBulkTransferForm = () => (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
            <div className="bg-white p-4 rounded-lg w-[400px] h-[400px] relative flex flex-col justify-between">
                <h2 className="text-2xl font-semibold mb-4">
                    Bàn giao {selectedAnalytes.length} chỉ tiêu
                </h2>
                        ...
                        {technicians.map((tech) => (
                            <div
                                ...
                                className={`p-2 mb-2 cursor-pointer border rounded ${
                                    selectedTechnician === tech.identity_uid
                                        ? "border-primary bg-blue-50"
                                        : "border-gray-300"
                                }`}
                                onClick={() =>
                                    setSelectedTechnician(tech.identity_uid)
                                }
                            >
                ...
                <div className="flex justify-end">
                    ...
                    <button
                        className={`${
                            selectedTechnician ? "bg-green-500" : "bg-gray-400"
        ...
        return (
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
                <div className="bg-white p-4 rounded-lg w-[400px] max-w-90vw">
                    ...
                    <p className="mb-6">
                        Mọi thay đổi của bạn sẽ thay đổi dữ liệu của thư viện
                        chỉ tiêu.
                    </p>
                    <div className="flex justify-end space-x-2">
                        <button
                            className="bg-gray-500 text-white px-4 py-2 rounded-lg"
                            onClick={cancelParameterUpdateMode}
                        >
                            Hủy bỏ
                        </button>
                        <button
                            className="bg-yellow-500 text-white px-4 py-2 rounded-lg"
                            onClick={confirmParameterUpdateMode}
                        >
                            Xác nhận
                        </button>
                    </div>
    ...
    const handleReviewAnalyses = async () => {
        ...
        try {
            ...
            const response = await apiPost(
                "https://black.irdop.org/trelw82ki/db/confirm/analysis",
                analysesToConfirm,
            );

            if (response.status === 200) {
                // Update the UI
                const currentUserName = await getIdenByUid(
                    currentUser.identity_uid,
                );
                const newAnalytesList = listAnalytes.map((analyte) => {
                    if (selectedAnalytes.includes(analyte.id)) {
                        return {
                ...
                showToast(
                    `Đã duyệt thành công ${selectedAnalytes.length} chỉ tiêu`,
                    "success",
                );
                ...
                Swal.fire({
                    ...
                    text:
                        response.data?.message ||
                        "Có lỗi xảy ra khi duyệt chỉ tiêu",
                });
            }
        } catch (error) {
    ...
    const handleConfirmBulkDeadline = async () => {
        try {
            ...
            if (failCount > 0) {
                ...
                showToast(
                    `Đã cập nhật hạn trả cho ${
                        selectedAnalytes.length
                    } chỉ tiêu thành ${formatDate(bulkDeadlineDate)}`,
                );
            }
            ...
            Swal.fire({
                ...
                text:
                    error.message ||
                    "An error occurred while updating deadlines",
            });
        }
    };
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
            <div className="bg-white p-4 rounded-lg w-[320px] relative">
                <h2 className="text-xl font-semibold mb-4">
                    Cập nhật hạn trả cho {selectedAnalytes.length} chỉ tiêu
                </h2>
                <div className="mb-4 flex justify-center">
                    <DatePicker
                        selected={bulkDeadlineDate}
                        onChange={(date) => setBulkDeadlineDate(date)}
                        inline
                    />
                </div>
                <div className="flex justify-end">
                    <button
                        className="bg-gray-500 text-white p-2 rounded mr-2"
                        onClick={() => setIsBulkDeadlineVisible(false)}
                    >
                        Hủy bỏ
                    </button>
                    <button
                        className="bg-green-500 text-white p-2 rounded"
                        onClick={handleConfirmBulkDeadline}
                    >
                        Xác nhận
                    </button>
                </div>
    ...
    return (
        <div className="w-full relative">
            ...
            <Breadcrumb
                ...
                sample_uids={listSampleByReceipt.map(
                    (sample) => sample.sample_uid,
                )}
            />
                    ...
                    <div className="flex md:flex-row flex-col md:justify-between items-center justify-center w-full">
                        <div className="w-full p-2 md:pr-4">
                            <table className="w-full border-none">
                                <tbody>
                                    <tr>
                                        <td className="w-1/5 text-start p-1 font-medium md:min-w-32 min-w-24 text-gray-500">
                                            Mã mẫu:
                                        </td>
                            ...
                            <div className="relative sample-dropdown-container">
                                ...
                                {sampleDropdownVisible && (
                                    <div className="absolute right-0 mt-1 bg-white border rounded shadow-lg z-10 min-w-40 sample-dropdown-container">
                                        <div className="p-2 bg-gray-100 font-medium">
                                            Chọn mẫu để sao chép thông tin:
                                        </div>
                                        <ul>
                                            {listSampleByReceipt.map(
                                                (sample) => (
                                                    <li
                                                        key={sample.sample_uid}
                    ...
                    <div className="min-w-80 whitespace-nowrap flex items-center flex-wrap py-1 pt-12 md:pt-0 md:pr-80 mr-0.5">
                        <div className="flex -translate-y-10 md:translate-y-0 md:pt-0 w-full justify-end">
                            ...
                            <button
                                ...
                                title={
                                    updateParameterMode
                                        ? "Đang cập nhật thư viện chỉ tiêu"
                                        : "Cập nhật thư viện chỉ tiêu"
                                }
                            >
                                <FaDatabase size={20} />
                            </button>
                    ...
                    <table className="text-black w-full border-2 analytes-table">
                        <thead>
                            <tr className="border-y-2">
                                <th className="p-2 border-x w-[100px] min-w-[100px] text-left">
                                    Mã chỉ tiêu{" "}
                                </th>
                                <th className="p-2 border-x w-[24%] min-w-60 text-left">
                                    Chỉ tiêu
                                </th>
                                {/* <th className="p-2 border-x w-32 min-w-32 text-left">Nền mẫu</th> */}
                                <th className="p-2 border-x w-[25%] min-w-44 text-left">
                                    Phương pháp
                                </th>
                                <th className="p-2 border-x w-[12%] min-w-28 text-left">
                                    Kết quả
                                </th>
                                <th className="p-2 border-x w-1/12 min-w-24 text-left">
                                    Đơn vị
                                </th>
                                <th className="p-2 border-x w-1/12 min-w-28 text-left">
                                    Hạn trả
                                </th>
                                <th className="p-2 border-2 w-[12%] min-w-36 text-left ">
                                    Thực hiện
                                </th>
                        ...
                        <tbody>
                            {isAddingNewParameter && (
                                <tr className="border bg-blue-50">
                                    ...
                                    <td className="p-1 border relative">
                                        {/* Kết quả - không nhập */}
                                        <div className="p-1 text-gray-400 italic text-center">
                                            --
                                        </div>
                                    </td>
                                    <td className="p-1 border relative">
                            ...
                            {listAnalytes?.map((order) => (
                                <tr key={order.id} className="border">
                                    ...
                                    <td
                                        className="p-1 border relative"
                                        onClick={() =>
                                            handleResultUnitClick(order)
                                        }
                                    >
                                        <div className="hover:border-purple-500 hover:border rounded ">
                                            {editingField ===
                                                `result_unit-${order.sample_id}-${order.id}` &&
                                            isEditorVisible ? (
                                                <TinyMceInput
                                                    value={inputValue || ""}
                                                    onUpdate={handleSaveContent}
                                                ...
                                                >
                                                    {technicians.map(
                                                        (identity) => (
                                                            ...
                                                            >
                                                                <p className="font-bold text-primary text-sm text-start">
                                                                    {identity.alias ||
                                                                        ""}
                                                                </p>
                                    ...
                                    <td className="pt-[5px] pb-0 border align-top text-center">
                                        <input
                                            type="checkbox"
                                            checked={selectedAnalytes.includes(
                                                order.id,
                                            )}
                                            onChange={() =>
                                                handleAnalyteSelect(order.id)
                                            }
