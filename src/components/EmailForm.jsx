...
const EmailForm = ({ receipt, isVisible, onClose }) => {
			...
			.map((sample, index) => {
				...
				if (sample.sample_information && sample.sample_information.length > 0) {
					...
					if (filteredInfo.length > 0) {
						sampleInfoLines = filteredInfo.map((info) => `   ${info.fname}: ${info.fvalue || ''}`).join('\n');
					}
				...
				if (sample.analysis && sample.analysis.length > 0) {
					const pairs = sample.analysis
						.map((analysis) => {
							...
							return parameter ? `      ${parameter} - ${method}` : '';
						})
						.filter((pair) => pair !== '');
				...
				let result = `${sampleNumber}. Mẫu thử: ${sample.sample_uid || ''}`;
   ...
   Yêu cầu: ${sample.additional_request || ''}`;
	...
	const generateEmailContent = (receiptData) => {
		...
		return `Kinh gửi quý khách hàng
...
Ngày trả kết quả dự kiến: ${receiptData?.deadline ? new Date(receiptData.deadline).toLocaleDateString('vi-VN') : ''}
Nơi trả kết quả: ${clientInfo.client_address || ''}

THÔNG TIN MẪU THỬ VÀ PHÂN TÍCH:
${sampleInfo}

Người liên hệ khi cần hỗ trợ: ${receiptData?.sale_recorder || currentUser?.fullname || ''}
...
- Thanh toán thành công đồng nghĩa với việc quy khách đồng ý với điều khoản và điều kiện dịch vụ kiểm nghiệm chi tiết tại: www.irdop.org/terms-and-conditions.
- Phương pháp thử sẽ do phòng thử nghiệm cung cấp dựa trên mẫu gửi.
