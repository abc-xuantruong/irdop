...
export default function MultiPageEditor() {
            ...
            if (sample_uid) {
                try {
                    const response = await apiGet(
                        `https://black.irdop.org/to82oe92i/db/sample/get/ppt_uid/${sample_uid}`,
                    );

                    if (response.status !== 200) {
                        throw new Error(
                            `PPT list API request failed with status ${response.status}`,
                        );
                    }
    ...
    const loadPublishedReport = async (reportId) => {
        ...
        try {
            const response = await apiGet(
                `https://black.irdop.org/to82oe92i/db/get/report/${reportId}`,
            );
            ...
            if (window.tinymce) {
                const headerElements =
                    document.getElementsByClassName("header-editable");
                if (headerElements.length > 0 && headerElements[0].id) {
                    const headerEditor = window.tinymce.get(
                        headerElements[0].id,
                    );
                    if (headerEditor) {
                        headerEditor.setContent(
                            reportData.header_section || header,
                        );
                    }
                }
            }
            ...
            if (window.tinymce) {
                const footerElements =
                    document.getElementsByClassName("footer-editable");
                if (footerElements.length > 0 && footerElements[0].id) {
                    const footerEditor = window.tinymce.get(
                        footerElements[0].id,
                    );
                    if (footerEditor) {
                        footerEditor.setContent(
                            reportData.footer_section || footer,
                        );
                    }
                }
            }
            ...
            // Allow editing if it's a DRAFT
            const isDraft =
                reportData.ppt_uid && reportData.ppt_uid.includes("DRAFT");
            ...
            showNotification(
                `Tải phiếu không thành công: ${err.message}`,
                "error",
            );
        } finally {
            setLoading(false);
    ...
    const fetchSampleData = async () => {
        ...
        try {
            setLoading(true);
            // Fetch sample data
            const sampleResponse = await apiGet(
            ...
            if (sampleResponse.status !== 200) {
                throw new Error(
                    `Sample API request failed with status ${sampleResponse.status}`,
                );
            }
            ...
            if (sampleResult.analysis && Array.isArray(sampleResult.analysis)) {
                const hasVlasProtocol = sampleResult.analysis.some(
                    (item) => item.protocol_source === "IRDOP VS",
                );
                if (hasVlasProtocol) {
                    setShowVlas(true);
                }
            ...
            if (sampleResult && sampleResult.receipt_id) {
                try {
                    ...
                    if (clientResponse.status !== 200) {
                        throw new Error(
                            `Client API request failed with status ${clientResponse.status}`,
                        );
                    }
    ...
    const updateContentWithData = (data) => {
        ...
        const updatedCommentSection = showComment
        ...
        if (!showReference && contentRef.current) {
            ...
            const extractCurrentReferences = () => {
                ...
                if (contentRef.current && window.tinymce) {
                    const contentEditor = window.tinymce.get(
                ...
                const referenceCells =
                    tempDiv.querySelectorAll(".reference-cell");

                if (referenceCells.length > 0) {
                    const refs = Array.from(referenceCells).map(
                        (cell) => cell.outerHTML,
                    );
...
<div style="padding-top: 0; display: flex; flex-direction: column; border: 1px solid #000000; margin:0;">
    <div style="padding: 5pt 8pt; flex-grow: 1; position: relative;">
		...
		<div style="display: flex; flex-direction: column; gap: 2px; height: fit-content;">
			...
			<p style="margin: 0; font-size: 12px; text-align: left; line-height: 1.2;">${
                clientAddress || "--"
            }</p>
		</div>
	</div>
    ...
    const generateSampleInfoSection = (data) => {
        ...
        const infoRows = sampleInfo
			...
			<div style="display: flex; ${
                fieldName.includes("Ngày tiếp nhận") && "margin-top: 8px;"
            }">
				...
				<div style="width: 70%; font-size: 12px; line-height: 1.2; text-align: left; padding-left: 10px;" >
					<p style="margin: 0; ${
                        mainLabel.toLowerCase().includes("tên mẫu")
                            ? "font-weight: bold;"
                            : ""
                    }">${fieldValue}</p>
				</div>
            ...
            if (contentRef.current && window.tinymce) {
                ...
                if (contentEditor) {
                    ...
                    const referenceCells =
                        tempDiv.querySelectorAll(".reference-cell");
                    if (referenceCells.length > 0) {
                        return Array.from(referenceCells).map(
                            (cell) => cell.outerHTML,
                        );
                    }
                }
        ...
        if (analysisItems.length > 0) {
            analysisRows = analysisItems
                .map((item, index) => {
                    ...
                    const protocol =
                        item?.protocol_source + " " + item.protocol_code ||
                        "--";
                    ...
                    if (showReference) {
                        ...
                        if (index < refsArray.length && refsArray[index]) {
                            ...
                            if (
                                refsArray[index].includes(
                                    'class="reference-cell"',
                                )
                            ) {
                                referenceCell = refsArray[index].replace(
                                    /padding: 6px 8px/g,
                                    "padding: 6px 10px",
                                );
                            } else {
                    ...
                    return `
				<tr>
					<td style="border: 1px solid black; padding: 4px 10px; text-align:left; font-size:12px;width:50px;">${
                        index + 1
                    }.</td>
...
<div class=" content_page_header_box" id="thead" style="position:relative; height: fit-content;">
    ...
    <div class=" " 
        ...
        <div class=" vlas_icon" 
             style="position:absolute; right:-5mm; top:0.2cm; ${
                 showVlas ? "" : "display:none;"
             }">
...
<div style="border-top:1px solid #4CB748; height:50px; display:flex; padding-top:0pt; align-items: center;">
    ...
    const initTinyMCE = () => {
        ...
        window.tinymce.init({
            ...
            setup: (editor) => {
                if (
                    editor.targetElm &&
                    editor.targetElm.classList &&
                    editor.targetElm.classList.contains("content-editable")
                ) {
                    ...
                    editor.on("init", () => {
                        ...
                        if (contentContainer) {
                            ...
                            if (isReadOnly) {
                                ...
                                contentContainer.style.WebkitUserSelect =
                                    "text";
                ...
                if (
                    editor.targetElm &&
                    editor.targetElm.classList &&
                    editor.targetElm.classList.contains("header-editable")
                ) {
                    // Only add change listeners if not in read-only mode
                    if (!isReadOnly) {
                    ...
                    editor.on("init", () => {
                        ...
                        if (
                            ...
                            headerContainer.lastElementChild.style.paddingBottom =
                                "0";
                            headerContainer.lastElementChild.style.marginBottom =
                                "0";
                        }
                ...
                if (
                    editor.targetElm &&
                    editor.targetElm.classList &&
                    editor.targetElm.classList.contains("footer-editable")
                ) {
                    // Only add change listeners if not in read-only mode
                    if (!isReadOnly) {
                        editor.on("change keyup input blur", () => {
                    ...
                    editor.on("init", () => {
                        ...
                        if (
                            footerContainer &&
                            footerContainer.firstElementChild
                        ) {
                            footerContainer.firstElementChild.style.paddingTop =
                                "0";
                            footerContainer.firstElementChild.style.marginTop =
                                "0";
                        }
    ...
    const handlePrint = () => {
        ...
        if (contentRef.current && window.tinymce) {
            ...
            if (headerElements.length > 0 && headerElements[0].id) {
                ...
                if (headerEditor) {
                    ...
                    const refCodeMatch = headerContent.match(
                        /<p[^>]*class="[^"]*ref_code[^"]*"[^>]*>(.*?)<\/p>/i,
                    );
                    if (refCodeMatch) {
                        headerContent = headerContent.replace(
                            refCodeMatch[0],
                            refCodeMatch[0].replace(
                                refCodeMatch[1],
                                pptUid || "-- SƠ BỘ / DRAFT --",
                            ),
                        );
        ...
        const printWindow = window.open(
            "",
            "_blank",
            "width=1100,height=1000,toolbar=no,scrollbars=yes",
        );
        if (!printWindow) {
        ...
        console.log(
            "- A4 paper:",
            `${A4.width}mm × ${A4.height}mm (${mmToPx(A4.width)}px × ${mmToPx(
                A4.height,
            )}px)`,
        );
        console.log(
            "- Margins:",
            `top: ${A4.topMargin}mm (${mmToPx(A4.topMargin)}px), bottom: ${
                A4.bottomMargin
            }mm (${mmToPx(A4.bottomMargin)}px), sides: ${
                A4.sideMargin
            }mm (${mmToPx(A4.sideMargin)}px)`,
        );
        ...
        function extractCurrentSections(currentContent) {
            ...
            const customerSectionDiv = Array.from(
                tempContainer.querySelectorAll("div"),
            ).find(
                (div) =>
                    div.innerHTML &&
                    div.innerHTML.includes("Customer information"),
            );
            ...
            if (customerSectionDiv) {
                ...
                extractedSections.customerSection =
                    customerSectionHTML || sectionContent.customerSection;
            }

            // Find sample info section (div containing "Sample information" text)
            const sampleInfoDiv = Array.from(
                tempContainer.querySelectorAll("div"),
            ).find(
                (div) =>
                    div.innerHTML &&
                    div.innerHTML.includes("Sample information"),
            );
            ...
            if (sampleInfoDiv) {
                ...
                extractedSections.sampleInfoSection =
                    sampleInfoSectionHTML || sectionContent.sampleInfoSection;
            }
            ...
            if (analysisTable && analysisTable.innerHTML.includes("Tests")) {
                ...
                while (
                    analysisParent.parentElement &&
                    analysisParent.parentElement !== tempContainer
                ) {
                ...
                extractedSections.analysisSection =
                    analysisSectionHTML || sectionContent.analysisSection;
            }
            ...
            const commentDiv = Array.from(
                ...
                (div) =>
                    div.innerHTML &&
                    div.innerHTML.includes("Nhận xét / Comment:"),
            );
            ...
            if (commentDiv) {
                ...
                while (
                    commentParent.parentElement &&
                    commentParent.parentElement !== tempContainer
                ) {
                ...
                extractedSections.commentSection = showComment
                    ? commentSectionHTML || sectionContent.commentSection
                    : "";
            }

            // Find notes section (div with "Note:" text)
            const notesDiv = Array.from(
                tempContainer.querySelectorAll("div"),
            ).find(
                (div) =>
                    div.innerHTML && div.innerHTML.includes("Ghi chú / Note:"),
            );
            ...
            if (notesDiv) {
                ...
                while (
                    notesParent.parentElement &&
                    notesParent.parentElement !== tempContainer
                ) {
                ...
                extractedSections.notesSection =
                    notesSectionHTML || sectionContent.notesSection;
            }

            // Find signature section (div with "Laboratory Manager" text)
            const signatureDiv = Array.from(
                tempContainer.querySelectorAll("div"),
            ).find(
                (div) =>
                    div.innerHTML &&
                    div.innerHTML.includes("Laboratory Manager"),
            );
            ...
            if (signatureDiv) {
                ...
                while (
                    signatureParent.parentElement &&
                    signatureParent.parentElement !== tempContainer
                ) {
                ...
                extractedSections.signatureSection =
                    signatureSectionHTML || sectionContent.signatureSection;
            }
        ...
        const paginateContent = () => {
            ...
            const sectionHeights = {
                customerSection: measureSection(
                    extractedSections.customerSection,
                ),
                sampleInfoSection: measureSection(
                    extractedSections.sampleInfoSection,
                ),
                analysisSection: measureSection(
                    extractedSections.analysisSection,
                ),
                commentSection: showComment
                    ? measureSection(extractedSections.commentSection || "")
                    : 0,
                notesSection: measureSection(extractedSections.notesSection),
                signatureSection: measureSection(
                    extractedSections.signatureSection,
                ),
            ...
            if (showComment && extractedSections.commentSection) {
                totalContentHeight +=
                    sectionHeights.commentSection + sectionHeights.spacing;
            }
            ...
            const totalExceedsOnePage =
                totalContentHeight > availableContentHeightPx;
            const page2FitsOnePage =
                page2SpecialLayoutHeight <= availableContentHeightPx;
            const page1FitsOnePage =
                page1SpecialLayoutHeight <= availableContentHeightPx;

            const useSpecialLayout =
                totalExceedsOnePage && page2FitsOnePage && page1FitsOnePage;
            ...
            if (useSpecialLayout) {
                ...
                console.log(
                    '📄 Using custom 2-page layout with "see next page" notification',
                );
                ...
                const processElement = (element) => {
                    ...
                    if (
                        ...
                        console.log(
                            `📊 Found table that needs splitting: ${elementHeightMm.toFixed(
                                2,
                            )}mm (exceeds available space ${pxToMm(
                                availableContentHeightPx - currentPageHeightPx,
                            ).toFixed(2)}mm)`,
                        );
                        splitTableAcrossPages(element);
                    } else if (
                        elementHeightPx > availableContentHeightPx &&
                        currentPage.length === 0
                    ) {
                        // Non-table element larger than a full page and we're at the start of a page
                        console.log(
                            `⚠️ Oversized non-table element: ${elementHeightMm.toFixed(
                                2,
                            )}mm (exceeds page height)`,
                        );
                        // Force onto a page
                        currentPage.push(
                            element.outerHTML || element.textContent,
                ...
                const splitTableAcrossPages = (tableElement) => {
                    ...
                    const tableHeader = hasHeader
                        ? tableElement.querySelector("thead").outerHTML
                        : "";
                    ...
                    if (currentPage.length > 0) {
                        const headerHeight = hasHeader
                            ? measureSection(
                                  `<table ${tableAttributes}>${tableHeader}</table>`,
                              )
                            : 0;
                        const minTableHeight =
                            headerHeight + (rows.length > 0 ? 50 : 0); // Minimum height for header + at least one row

                        if (
                            currentPageHeightPx + minTableHeight >
                            availableContentHeightPx
                        ) {
                    ...
                    const fullTableHTML = `<table ${tableAttributes}>${
                    ...
                    const renderedRows = Array.from(
                        measureArea.querySelectorAll("tbody tr"),
                    );

                    // Log table information
                    console.log(`📏 TABLE ROWS HEIGHT MEASUREMENT:`);
                    console.log(
                        `- Available space for rows: ${remainingHeightPx}px (${pxToMm(
                            remainingHeightPx,
                        ).toFixed(2)}mm)`,
                    );
                    ...
                    const rowHeights = renderedRows.map((row, index) => {
                        ...
                        const percentOfAvailable =
                            (rowHeightPx / availableContentHeightPx) * 100;

                        // Log detailed information about this row with both original and adjusted heights
                        console.log(
                            `- Row ${
                                index + 1
                            }: Original ${originalRowHeightPx.toFixed(
                                1,
                            )}px, Adjusted ${rowHeightPx.toFixed(
                                1,
                            )}px (${rowHeightMm.toFixed(
                                2,
                            )}mm) - ${percentOfAvailable.toFixed(
                                1,
                            )}% of available space`,
                        );
                    ...
                    for (
                        let i = 0;
                        i < rows.length && i < rowHeights.length;
                        i++
                    ) {
                        ...
                        if (rowHeightPx <= totalRemainingHeight) {
                            ...
                            remainingRows.shift();
                            console.log(
                                `- Row ${i + 1} fits: ${rowHeightPx.toFixed(
                                    1,
                                )}px - Remaining space: ${totalRemainingHeight.toFixed(
                                    1,
                                )}px (${pxToMm(totalRemainingHeight).toFixed(
                                    2,
                                )}mm)`,
                            );
                        } else {
                            ...
                            console.log(
                                `- Row ${
                                    i + 1
                                } doesn't fit: ${rowHeightPx.toFixed(
                                    1,
                                )}px > ${totalRemainingHeight.toFixed(
                                    1,
                                )}px remaining`,
                            );
                            break;
                    ...
                    console.log(
                        `- Total rows that fit: ${rowsInFirstPart.length} of ${rows.length}`,
                    );
                    console.log(
                        `- Total height used: ${totalUsedHeight.toFixed(
                            1,
                        )}px (${pxToMm(totalUsedHeight).toFixed(2)}mm)`,
                    );
                    console.log(
                        `- Remaining height: ${totalRemainingHeight.toFixed(
                            1,
                    ...
                    if (rowsInFirstPart.length > 0) {
                        ...
                        if (remainingRows.length > 0) {
                            ...
                            for (let i = 0; i < remainingRows.length; i++) {
                                ...
                                const originalRowHeightPx =
                                    ...
                                    rowIndex < rowHeights.length
                                        ? rowHeights[rowIndex] / 0.999
                                        : 30; // Convert back to original height for display purposes only
                                const rowHeightPx =
                                    rowIndex >= 0 &&
                                    rowIndex < rowHeights.length
                                        ? rowHeights[rowIndex]
                                        : 30 * 0.999; // Default height if not found, with 12% reduction
                                ...
                                if (
                                    ...
                                    console.log(
                                        `  - Row added: Original height ${originalRowHeightPx.toFixed(
                                            1,
                                        )}px, Used ${rowHeightPx.toFixed(
                                            1,
                                        )}px, Remaining space: ${(
                                            availableContentHeightPx -
                                            currentPageHeightPx
                            ...
                            if (remainingRows.length > 0) {
                                ...
                                if (hasHeader)
                                    remainingTableHTML += tableHeader;
                ...
                pageContentHeights.forEach((height, index) => {
                    ...
                    const percentUsed =
                        (height / availableContentHeightPx) * 100;
                    const remainingPx = availableContentHeightPx - height;
                    const remainingMm = pxToMm(remainingPx);

                    console.log(
                        `- Page ${index + 1}: Content height = ${height.toFixed(
                            1,
                        )}px (${heightMm.toFixed(2)}mm)`,
                    );
                    console.log(
                        `  • ${percentUsed.toFixed(
                            1,
                        )}% of available space used`,
                    );
                    console.log(
                        `  • Remaining space: ${remainingPx.toFixed(
                            1,
                        )}px (${remainingMm.toFixed(2)}mm)`,
                    );
                });

                // Verify our actual page count
                console.log(
                    `📄 Standard layout resulted in ${contentPages.length} pages`,
                );
        ...
        printWindow.document.write(`
				...
				<head>
					...
					<style>
						...
						@page {
							size: A4;
							margin: ${A4.topMargin}mm ${A4.sideMargin}mm ${A4.bottomMargin}mm ${
            A4.sideMargin
        }mm;
						}
        ...
        const printContainer =
            printWindow.document.querySelector(".print-container");
        ...
        paginationResult.pages.forEach((pageContent, index) => {
            ...
            const percentOfAvailable =
                (actualContentHeight /
                    paginationResult.availableContentHeightPx) *
                100;
            const remainingSpace =
                paginationResult.availableContentHeightPx - actualContentHeight;
            const remainingSpaceMm = pxToMm(remainingSpace);

            console.log(
                `📃 Final Page ${pageNumber}/${totalPages} content measurements:`,
            );
            console.log(
                `- Content height: ${actualContentHeight.toFixed(
                    1,
                )}px (${actualContentHeightMm.toFixed(2)}mm)`,
            );
            console.log(
                `- ${percentOfAvailable.toFixed(1)}% of available space used`,
            );
            console.log(
                `- Remaining space: ${remainingSpace.toFixed(
                    1,
                )}px (${remainingSpaceMm.toFixed(2)}mm)`,
            );
        ...
        console.log(
            "🖨️ Print preparation complete with",
            paginationResult.pages.length,
            "pages",
        );
        console.log(
            "🔍 Using layout:",
            paginationResult.is2PageLayout
                ? "Custom 2-page layout"
    ...
    const publishReport = async (type) => {
        try {
                ...
                if (contentRef.current && window.tinymce) {
                    ...
                    if (contentEditor) {
                        ...
                        const referenceCells =
                            tempDiv.querySelectorAll(".reference-cell");
                        if (referenceCells.length > 0) {
                            return Array.from(referenceCells).map((cell) =>
                                cell.innerHTML.trim(),
                            );
                        }
                    }
            ...
            console.log(
                `${
                    type === "publish" ? "Publishing" : "Saving"
                } report with data:`,
                requestBody,
            );
            ...
            const response = await apiPost(
                "https://black.irdop.org/to82oe92i/db/insert/ppt",
                {
                    report: requestBody,
            ...
            console.log(
                `${type === "publish" ? "Published" : "Saved"} successfully:`,
                result,
            );
            ...
            if (result && result.ppt_uid) {
                ...
                showNotification(
                    `${
                        type === "publish" ? "Phát hành" : "Lưu"
                    } phiếu phân tích thành công!`,
                    "success",
                );
                ...
                showNotification(
                    `${
                        type === "publish" ? "Phát hành" : "Lưu"
                    } không thành công, không nhận được mã phiếu`,
                    "error",
                );
            }
        } catch (err) {
            console.error(
                `Error ${type === "publish" ? "publishing" : "saving"} report:`,
                err,
            );
    ...
    function extractCurrentSections(currentContent) {
        ...
        const customerSectionDiv = Array.from(
            ...
            (div) =>
                div.innerHTML && div.innerHTML.includes("Customer information"),
        );
        ...
        const sampleInfoDiv = Array.from(
            ...
            (div) =>
                div.innerHTML && div.innerHTML.includes("Sample information"),
        );
        ...
        const commentDiv = Array.from(
            ...
            (div) =>
                div.innerHTML && div.innerHTML.includes("Nhận xét / Comment:"),
        );
        ...
        const signatureDiv = Array.from(
            ...
            (div) =>
                div.innerHTML && div.innerHTML.includes("Laboratory Manager"),
        );
    ...
    useEffect(() => {
        ...
        const handleReferenceToggle = (prevShowRef, newShowRef) => {
                ...
                if (contentRef.current && window.tinymce) {
                    ...
                    if (contentEditor) {
                        ...
                        if (referenceCells.length > 0) {
                            ...
                            console.log(
                                `Reference column turned OFF: Saved ${extractedRefs.length} reference values`,
                                extractedRefs,
                            );
            ...
            if (!prevShowRef && newShowRef) {
                console.log(
                    `Reference column turned ON: Using ${referenceValues.length} stored reference values`,
                );
            }
        };
        ...
        <div className="p-4 bg-gray-100 min-h-screen relative mt-1">
            <div className="mb-4 flex flex-col gap-4 items-end min-w-[800px]">
                ...
                <div className="flex justify-between items-center w-full">
                    ...
                    <div>
                        <button
                            onClick={() =>
                                !isReadOnly && setShowVlas((prev) => !prev)
                            }
                            className={`${
                                showVlas
                        ...
                        <button
                            onClick={() =>
                                !isReadOnly && setShowComment((prev) => !prev)
                            }
                            className={`${
                                showComment
                                    ? "bg-sky-500 text-white"
            ...
            {additionalRequest && (
                <div className="fixed right-4 text-start top-96 w-56 max-h-60 overflow-y-auto bg-green-50 rounded-lg border border-green-200 shadow-md p-3 z-10">
                    ...
                    <div className="text-blue-600 text-sm whitespace-pre-wrap">
                        {additionalRequest}
                    </div>
                    {/* Simple arrow using a div instead of pseudo-element */}
            ...
            <style jsx>{`
                @font-face {
                    font-family: "Gilroy";
                    src: url("/public/fonts/SVN-Gilroy Regular.otf")
                        format("opentype");
                    font-weight: 400;
                    font-style: normal;
                    font-display: swap;
                ...
                @font-face {
                    font-family: "Gilroy";
                    src: url("/public/fonts/SVN-Gilroy SemiBold.otf")
                        format("opentype");
                    font-weight: 500;
                    font-style: normal;
                    font-display: swap;
                ...
                @font-face {
                    font-family: "Gilroy";
                    src: url("/public/fonts/SVN-Gilroy Bold.otf")
                        format("opentype");
                    font-weight: 700;
                    font-style: normal;
                    font-display: swap;
