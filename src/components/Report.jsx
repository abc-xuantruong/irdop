...
export default function MultiPageEditor() {
    ...
    const loadPublishedReport = async (reportId) => {
        ...
        try {
            ...
            if (
                (!reportData.receipt_note || !reportData.additional_request) &&
                sample_uid
            ) {
                try {
                    ...
                    if (sampleResponse.status === 200) {
                        ...
                        if (
                            !reportData.additional_request &&
                            sampleData.additional_request
                        ) {
    ...
    const generateCombinedContent = (
        ...
        const commentContent =
            showComment && commentSection ? commentSection : "";
    ...
    useEffect(() => {
        const fetchRelatedSamples = async () => {
            if (sample_uid) {
                try {
                    const response = await apiGet(
                        `https://black.irdop.org/to82oe92i/db/get/sample_by_sample_uid/${sample_uid}`,
                    );
                    if (response.status === 200) {
    ...
    const updateContentWithData = (data) => {
        ...
        const updatedCommentSection = showComment
            ? commentSectionHTML || generateCommentSection()
            : "";
                ...
                const heights = Array.from(rows)
                    .map((rowRef, index) => {
                        ...
                        try {
                            ...
                            const boundingHeight = boundingClientRect
                                ? boundingClientRect.height
                                : 0;

                            // Get the computed style for additional measurements
                            const computedStyle =
                                window.getComputedStyle(rowRef);

                            // Calculate padding, border, margin heights
                            const paddingTop = safeParseFloat(
                                computedStyle.paddingTop,
                            );
                            const paddingBottom = safeParseFloat(
                                computedStyle.paddingBottom,
                            );
                            const borderTopWidth = safeParseFloat(
                                computedStyle.borderTopWidth,
                            );
                            const borderBottomWidth = safeParseFloat(
                                computedStyle.borderBottomWidth,
                            );
                            const marginTop = safeParseFloat(
                                computedStyle.marginTop,
                            );
                            const marginBottom = safeParseFloat(
                                computedStyle.marginBottom,
                            );

                            // Add computation of the height from computed style
                            const computedHeight = safeParseFloat(
        ...
        const getCurrentReferenceValues = () => {
            if (contentRef.current && window.tinymce) {
                ...
                if (contentEditor) {
                    ...
                    if (editorBody) {
                        const referenceCells =
                            editorBody.querySelectorAll(".reference-cell") ||
                            [];
                        if (referenceCells.length > 0) {
        ...
        if (analysisItems.length > 0) {
            analysisRows = analysisItems
                .map((item, index) => {
                    ...
                    const scope =
                        protocolSource +
                        (accreditationParts.length > 0
                            ? " " + accreditationParts.join(" ")
                            : "");
				...
				<tr id="${rowId}" class="table-row" data-row-index="${index}">
					<td style="border: 1px solid black; padding: 4px 8px; text-align:left; font-size:12px;">${
                        index + 1
                    }.</td>
...
<div class=" content_page_header_box" id="thead" style="position:relative; height: fit-content;">
	...
	<div class=" " 
		...
		<div class=" vlas_icon" 
			 style="position:absolute; right:0mm; top:0.2cm; ${
...
<div style="display: flex; align-items: center; height: 14px;"><span style="font-size: 11px; margin: 0; padding: 0; line-height: 1;">00</span> <span style="font-size: 11px; margin: 0; padding: 0; line-height: 1;">/</span> <span style="font-size: 11px; margin: 0; padding: 0; line-height: 1;">00</span></div>
    ...
    const extractTableDimensions = (shouldLog = false) => {
        ...
        const rowDimensions = Array.from(rows).map((tr, rowIndex) => {
            ...
            const cellDimensions = Array.from(cells).map((td, cellIndex) => {
                if (!td)
                    return { rowIndex, cellIndex, width: "0px", height: "0px" };
    ...
    const handlePrint = () => {
        ...
        if (contentRef.current && window.tinymce) {
            ...
            if (contentEditor) {
                ...
                const tableHeaderWidths = extractTableHeaderWidths(
                    contentEditor.getBody(),
                );
                ...
                if (tableHeaderWidths && tableHeaderWidths.length > 0) {
                    currentContent = applyTableHeaderWidths(
                        currentContent,
                        tableHeaderWidths,
                    );
                }
        ...
        function extractCurrentSections(currentContent) {
            ...
            if (
                ...
                analysisTable.innerHTML.includes("Tests")
            ) {
        ...
        const paginateContent = () => {
            ...
            const sectionHeights = {
                ...
                nextPageNotification: measureSection(
                    nextPageNotification,
                    "PAGE BREAK NOTIFICATION",
                ),
            };
            console.groupEnd();
            ...
            if (useSpecialLayout) {
                ...
                const splitTableAcrossPages = (
                    ...
                    if (recursionDepth > 10) {
                        currentPage.push(
                            `<div style="color: red; padding: 5px;">Table pagination limit reached</div>`,
                        );
                        return;
                    }
                    ...
                    rows.forEach((row, index) => {
                        ...
                        const boundingHeight =
                            measurableRow.getBoundingClientRect().height || 0;

                        // Choose the most accurate measurement, preferring offsetHeight if available
                        let bestHeight =
                            offsetHeight > 0 ? offsetHeight : boundingHeight;

                        // If we still don't have a reliable height, use the computed style approach
                        if (bestHeight < 5) {
                            const computedStyle =
                                window.getComputedStyle(measurableRow);
                            const computedHeight =
                                parseFloat(computedStyle.height) || 0;
                            bestHeight =
                                computedHeight > 0 ? computedHeight : 32; // Default to 32px if still no height
                        }
            ...
            contentPages.forEach((pageContent, index) => {
                ...
                tempMeasureDiv.style.width = `${
                    A4.width - 2 * A4.sideMargin
                }mm`;
                ...
                const percentOfAvailable =
                    (actualContentHeight / availableContentHeightPx) * 100;
                const remainingSpace =
                    availableContentHeightPx - actualContentHeight;
                const remainingSpaceMm =
                    availableContentHeightMm - actualContentHeightMm;
                ...
                if (typeof pageContent === "string") {
                    ...
                    const hasSection = (sectionHtml) => {
                        return pageContent.includes(
                            sectionHtml.substring(0, 100),
                        );
                    };

                    if (hasSection(extractedSections.customerSection)) {
                        console.log(
                            `- Customer section: ${sectionHeights.customerSection.toFixed(
                                1,
                            )}px (${pxToMm(
                                sectionHeights.customerSection,
                            ).toFixed(2)}mm)`,
                    ...
                    if (hasSection(extractedSections.sampleInfoSection)) {
                        console.log(
                            `- Sample info section: ${sectionHeights.sampleInfoSection.toFixed(
                                1,
                            )}px (${pxToMm(
                                sectionHeights.sampleInfoSection,
                            ).toFixed(2)}mm)`,
                    ...
                    if (hasSection(extractedSections.analysisSection)) {
                        ...
                        const tableRows =
                            tempDiv.querySelectorAll("table tbody tr");
                        console.log(
                            `- Analysis table: ${sectionHeights.analysisSection.toFixed(
                                1,
                            )}px (${pxToMm(
                                sectionHeights.analysisSection,
                    ...
                    if (
                        showComment &&
                        hasSection(extractedSections.commentSection)
                    ) {
                        console.log(
                            `- Comment section: ${sectionHeights.commentSection.toFixed(
                                1,
                            )}px (${pxToMm(
                    ...
                    if (hasSection(extractedSections.notesSection)) {
                        console.log(
                            `- Notes section: ${sectionHeights.notesSection.toFixed(
                                1,
                            )}px (${pxToMm(sectionHeights.notesSection).toFixed(
                                2,
                            )}mm)`,
                        );
                    }
                    ...
                    if (hasSection(extractedSections.signatureSection)) {
                        console.log(
                            `- Signature section: ${sectionHeights.signatureSection.toFixed(
                                1,
                            )}px (${pxToMm(
                                sectionHeights.signatureSection,
                            ).toFixed(2)}mm)`,
				...
				<head>
					...
					<style>
						...
						.page {
							...
							padding: ${A4.topMargin * 3.78}px ${A4.sideMargin * 3.78}px ${
            A4.bottomMargin * 3.78
        }px ${A4.sideMargin * 3.78}px;
							overflow: hidden;
						}
						...
						.content {
							position: absolute;
							top: calc(${A4.topMargin * 3.78}px + ${paginationResult.headerHeightPx}px + ${
            A4.headerSpacing * 3.78
        }px);
							left: ${A4.sideMargin * 3.78}px;
							right: ${A4.sideMargin * 3.78}px;
							width: calc(100% - ${2 * A4.sideMargin * 3.78}px);
    ...
    const performDetailedTableMeasurements = () => {
        ...
        const headerHeight =
            document.getElementById("header-edit")?.offsetHeight || 0;
        const footerHeight =
            document.getElementById("footer-edit")?.offsetHeight || 0;
        ...
        const tableHeader = document.querySelector(
            ".content-editable table thead",
        );
        const tableHeaderHeight = tableHeader
            ? tableHeader.getBoundingClientRect().height
            : 0;
        console.log(
            `Table header height: ${tableHeaderHeight.toFixed(1)}px (${pxToMm(
                tableHeaderHeight,
            ).toFixed(2)}mm)`,
        );
        ...
        console.log(
            `Remaining height for rows: ${remainingHeight.toFixed(
                1,
            )}px (${pxToMm(remainingHeight).toFixed(2)}mm)`,
        );
            ...
            if (row.cells && row.cells.length > 0) {
                ...
                Array.from(row.cells).forEach((cell, cellIndex) => {
                    ...
                    if (cellHeight > maxCellHeight) {
                        ...
                        tallestCellStyle =
                            computedStyleToObject(cellComputedStyle);
                    }
                });
            }
    ...
    return (
        <div className="p-4 bg-gray-100 min-h-screen relative mt-1">
            <div className="flex flex-col w-fit mb-2">
                <h2 className="text-4xl font-medium text-primary text-start">
                    Phiếu phân tích
                </h2>
