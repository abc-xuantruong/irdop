        ...
        <div className="w-full h-full relative bg-gray-50">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                        {selectedAnalyses.length > 0 && (
                            <>
                                <button className="px-3 py-1.5 text-sm border border-gray-300 text-gray-700 rounded hover:bg-gray-50" onClick={handleCancelSelection}>
                                    Hủy chọn ({selectedAnalyses.length})
                                </button>
                                <button className="px-3 py-1.5 text-sm bg-blue-500 text-white rounded hover:bg-blue-600" onClick={handleBulkEditClick}>
                                    Sửa hàng loạt
                                </button>
                            </>
                        )}
                        ...
                        {isResultEntrySession ? (
                            <>
                                <button className="px-3 py-1.5 text-sm bg-green-600 text-white rounded hover:bg-green-700 flex items-center" onClick={handleEndSession}>
                                    <span className="mr-2">📝</span>
                                    Kết thúc nhập ({pendingChangesCount})
                                </button>
                            </>
                        ) : (
                            <button className="px-3 py-1.5 text-sm border border-blue-600 text-blue-600 rounded hover:bg-blue-50" onClick={handleStartSession}>
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-sm rounded transition-colors ${
                                isUrgentFilterActive ? "bg-red-600 text-white border border-red-600 hover:bg-red-700" : "border border-red-600 text-red-600 hover:bg-red-50"
                            }`}
                        >
                            Mẫu khẩn
                        </button>
                        <button className="px-3 py-1.5 text-sm border border-green-600 text-green-600 rounded hover:bg-green-50">Đủ kết quả</button>
                    </div>
                </div>
            </div>
            ...
            <LabBulkUpdate
                ...
                onUpdateTableData={(bulkChanges) => {
                    ...
                    // This callback is for compatibility with LabBulkUpdate
                }}
                onStartSession={() => {
