...
const ConfirmLabResult = ({ isOpen, onClose, onConfirm, onCancel, analyses = [], isLoading = false, originalAnalyses = [] }) => {
    ...
    const generateLabelPrintHTML = (allSampleIds) => {
        ...
        const generateCopyId = () => {
            ...
            const random = Array.from({ length: 4 }, () => "abcdefghijklmnopqrstuvwxyz".charAt(Math.floor(Math.random() * 26))).join("");
        ...
        const rowsHTML = rows
            .map((row, rowIndex) => {
                ...
                const sampleTspans = sampleLines.map((line, index) => `<tspan x="0" dy="${index === 0 ? "0" : "1mm"}">${line}</tspan>`).join("");

                // Right label with copyId
                const copyIdTspans = `<tspan x="2" dy="0">CopyID:</tspan><tspan x="2" dy="1.8mm">${row.copyId}</tspan>`;

                return `
					<div class="row" data-row-index="${rowIndex}" data-sample-ids='${JSON.stringify(row.sampleIds)}' data-all-sample-ids='${JSON.stringify(allSampleIds)}'>
    ...
    const handleExtractAndPrint = async () => {
        try {
            ...
            if (currentContent && currentContent.trim() !== "") {
                const { measureContentInDOM, paginateContent, generatePreviewHTML } = await import("../../../contexts/documentPreviewHelpers");
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-4 h-[20px]">
                                            <div className="flex items-center gap-2">
                                                ...
                                                />
                                                <label htmlFor="hasExperimentLog" className="text-sm font-semibold text-gray-700 cursor-pointer">
                                                    Có mã nhật ký
                                                </label>
                                            </div>
                                            <div className="flex items-center gap-2">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <div>
                                                ...
                                                />
                                                {experimentStartDateError && <p className="text-red-500 text-xs mt-1">{experimentStartDateError}</p>}
                                            </div>
                                            <div>
                                                <input
                                                    type="text"
                                                ...
                                                />
                                                {experimentEndDateError && <p className="text-red-500 text-xs mt-1">{experimentEndDateError}</p>}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <table className="w-full text-sm">
                                        <thead className="bg-gray-100 sticky top-0">
                                            <tr>
                                                ...
                                                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-700 border-b">Phương pháp</th>
                                            ...
                                            {analyses.map((analysis, index) => (
                                                <tr key={index} className="hover:bg-gray-50 text-start">
                                                    ...
                                                    <td className="px-3 py-2">
                                                        <div className="text-gray-900" dangerouslySetInnerHTML={{ __html: analysis.resultValue || "--" }} />
                                                    </td>
                                                    <td className="px-3 py-2">
                                                        <div className="text-gray-900" dangerouslySetInnerHTML={{ __html: analysis.resultUnit || "--" }} />
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                            ...
                            <div className="flex-1 overflow-hidden border border-gray-300 rounded-lg">
                                <TinyMCEEditor
                                    ...
                                    init={{
                                        ...
                                        toolbar: "blocks fontfamily fontsize bold italic underline strikethrough subscript superscript forecolor align lineheight numlist bullist indent outdent table",
                            ...
                            {hasResultValueChanges ? (
                                <>
                                    <div className="mb-4 p-3 bg-orange-50 border border-orange-300 rounded-lg">
                                        ...
                                        <p className="text-sm text-orange-700 text-center">Vì có thay đổi giá trị kết quả (resultValue), bạn bắt buộc phải trích xuất và tải xuống báo cáo PDF.</p>
                                    </div>
                                    <p className="text-sm text-gray-500 text-center">Hệ thống sẽ tạo file PDF đã phân trang và tự động tải xuống</p>
                                </>
                            ) : (
                                <>
                                    <p className="text-gray-700 text-center mb-4">Bạn có muốn trích xuất báo cáo PDF và tải xuống không?</p>
                                    <p className="text-sm text-gray-500 text-center">Nếu chọn "Có", hệ thống sẽ tạo file PDF đã phân trang và tự động tải xuống</p>
                                </>
                            )}
                        </div>
