...
const LabBulkUpdate = ({
    ...
    onUpdateTableData, // Callback to update table display data immediately
    ...
    const handleBulkUpdate = () => {
        ...
        selectedIds.forEach((analysisId) => {
            ...
            if (hasChanges) {
                ...
                // Thay vì chỉ push changeData (partial), merge với full analysis object
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg min-w-[600px] w-5/6 max-h-[90vh] overflow-auto">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold">
                        Chỉnh sửa hàng loạt
                        <span className="text-sm font-normal text-gray-600 ml-2">({Array.isArray(selectedRows) ? selectedRows.length : selectedRows.size} chỉ tiêu được chọn)</span>
                    </h2>
                    ...
                    <div className="flex flex-wrap gap-4 items-end">
                        ...
                        <div className="flex-grow" style={{ minWidth: "150px" }}>
                            ...
                            {!editingField && bulkEditValues.resultValue && (
                                <div className="text-xs text-gray-500 mt-1 p-1 bg-gray-50 rounded" dangerouslySetInnerHTML={{ __html: bulkEditValues.resultValue }} />
                            )}
                        </div>
                        ...
                        <div className="flex-grow relative" style={{ minWidth: "120px" }}>
                            ...
                            {!editingField && bulkEditValues.resultUnit && (
                                <div className="text-xs text-gray-500 mt-1 p-1 bg-gray-50 rounded" dangerouslySetInnerHTML={{ __html: bulkEditValues.resultUnit }} />
                            )}
                            {showUnitDropdown &&
                                    ...
                                    >
                                        ...
                                        {filterUnits(unitInput).length > itemsPerPage && (
                                            <div className="flex justify-between p-2 bg-gray-100">
                                                <button className="px-2 py-1 border rounded disabled:opacity-50" onClick={() => handleUnitPageChange(unitPage - 1)} disabled={unitPage === 1}>
                                                    Prev
                                                </button>
                ...
                <div className="mb-6">
                    <h3 className="text-md font-semibold mb-3">Xem trước thay đổi ({Array.isArray(selectedRows) ? selectedRows.length : selectedRows.size} mục)</h3>
                                ...
                                {normalizedSelectedData.map((foundAnalysis) => (
                                    <tr key={foundAnalysis.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="border border-gray-300 p-2 text-sm text-start">
                                            <span className={hasFieldChanged("protocolSource", foundAnalysis.protocolSource, bulkEditValues.protocolSource) ? "font-semibold text-blue-600" : ""}>
                                                {bulkEditValues.protocolSource || foundAnalysis.protocolSource || "--"}
                                            </span>
                                        </td>
                                        <td className="border border-gray-300 p-2 text-sm text-start">
                                            <span className={hasFieldChanged("protocolCode", foundAnalysis.protocolCode, bulkEditValues.protocolCode) ? "font-semibold text-blue-600" : ""}>
                                                {bulkEditValues.protocolCode || foundAnalysis.protocolCode || "--"}
                                            </span>
                                        </td>
                                        <td className="border border-gray-300 p-2 text-sm text-start">
                                            <div
                                                className={hasFieldChanged("resultValue", foundAnalysis.resultValue, bulkEditValues.resultValue) ? "font-semibold text-blue-600" : ""}
                                        ...
                                        <td className="border border-gray-300 p-2 text-sm text-start">
                                            <div
                                                className={hasFieldChanged("resultUnit", foundAnalysis.resultUnit, bulkEditValues.resultUnit) ? "font-semibold text-blue-600" : ""}
                                                dangerouslySetInnerHTML={{
                                                    __html: bulkEditValues.resultUnit || foundAnalysis.resultUnit || "--",
                                                }}
                                            />
                                        </td>
                                        <td className="border border-gray-300 p-2 text-sm text-start">{formatDate(foundAnalysis.deadline)}</td>
                                        <td className="border border-gray-300 p-2 text-sm text-start">
                                            <span className={hasFieldChanged("technicianId", foundAnalysis.technicianId, bulkEditValues.technicianId) ? "font-semibold text-blue-600" : ""}>
                                                {bulkEditValues.technicianId ? getTechnicianName(bulkEditValues.technicianId) : getTechnicianName(foundAnalysis.technicianId)}
                                            </span>
                                        </td>
                                    </tr>
                                ))}
                ...
                <div className="flex justify-end space-x-3">
                    <button onClick={handleClose} className="px-4 py-2 text-sm border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors">
                        Hủy
                    </button>
                    <button
                ...
                <div className="mt-4 text-xs text-gray-500">
                    <strong>Lưu ý:</strong> Các thay đổi sẽ được lưu tạm thời vào phiên nhập liệu. Bạn cần <strong>kết thúc phiên nhập</strong> để lưu các thay đổi vào cơ sở dữ liệu. Các trường màu
                    xanh dương trong bảng xem trước cho biết sẽ có thay đổi.
                </div>
            </div>
        </div>
    );
