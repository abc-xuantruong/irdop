...
const Dashboard = () => {
    ...
    const { setCurrentTitlePage, status, purposes, formatDate, getIdenByUid, identityCache, currentUser } = useContext(GlobalContext);
    ...
    // Add new state to track result filled filter
    const [showResultFilledFilter, setShowResultFilledFilter] = useState(false);

    // Add new state to track reviewed filter
    const [showReviewedFilter, setShowReviewedFilter] = useState(false);
    ...
    const fetchReceiptsByDeadline = async (start, end) => {
        try {
            ...
            const formattedEndDate = end ? `${end.getFullYear()}-${String(end.getMonth() + 1).padStart(2, "0")}-${String(end.getDate()).padStart(2, "0")}` : null;
            ...
            if (response.status === 200) {
                if (response.data && response.data.result && Array.isArray(response.data.result)) {
                    ...
                    setTotalPages(pagination.totalPages || Math.ceil((pagination.totalItems || response.data.result.length) / receiptsPerPage));
                    ...
                    showToast(`Hiển thị ${displayCount} trong số ${totalCount} tiếp nhận có hạn trả kết quả từ ${formatDate(start)} đến ${formatDate(end)}`, "info");
                } else {
    ...
    const fetchReceipt = async (page = null, itemsPerPage = receiptsPerPage) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                const hasActiveResultsFilledFilter = location.search.includes("resultsFilled=");
    ...
    useEffect(() => {
        ...
        } else if (queryParams.get("resultsFilled") === "true") {
            // Results filled filter is active
            setShowResultFilledFilter(true);
            setShowOverdueFilter(false);
            setShowTodayDeadlines(false);
            setShowReviewedFilter(false);
    ...
    }, [isFilter, searchTerm, statusFilter, recordCodeSort, requestNumberSort, currentPage, receiptsPerPage, location.search]); // Re-run effect when any filter state changes
            ...
            if (response?.status < 300) {
                ...
                setCurrentList((prevList) =>
                    prevList.map((receipt) => ({
                        ...
                        samples: receipt.samples?.map((sample) => ({
                            ...sample,
                            analyses: sample.analyses?.map((analysis) => (analysis.id === selectedAnalysisForNote.id ? { ...analysis, note: newNote } : analysis)),
                        })),
                    })),
                );
                    ...
                    if (searchParam) {
                        ...
                        const filtered = originalList.filter(
                            (receipt) =>
                                ...
                                receipt.samples?.some((sample) => (sample.sampleId || "").includes(searchParam) || (sample.sampleName || "").toLowerCase().includes(searchParam.toLowerCase())),
                        );
    ...
    return (
        ...
        <div className="flex flex-col justify-between items-center w-full">
            ...
            {analysisSummaryTooltip.visible && analysisSummaryTooltip.analyses.length > 0 && (
                ...
                >
                    <div className="grid gap-2 font-semibold mb-2 pb-2 border-b text-left" style={{ gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 0.5fr" }}>
                    ...
                    <div className="grid gap-2 max-h-[250px] overflow-y-auto text-left" style={{ gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr 0.5fr" }}>
                        {analysisSummaryTooltip.analyses.map((analysis, idx) => (
                            <React.Fragment key={`summary-${idx}`}>
                                <div className="truncate whitespace-nowrap overflow-hidden text-ellipsis" title={analysis.parameterName || "--"}>
                                    {analysis.parameterName || "--"}
                                </div>
                                <div className="truncate whitespace-nowrap overflow-hidden text-ellipsis" title={analysis.protocolCode || "--"}>
                                    {analysis.protocolCode || "--"}
                                </div>
                                <div className="truncate whitespace-nowrap overflow-hidden text-ellipsis" title={analysis.resultValue || "--"}>
                                ...
                                <div className="truncate whitespace-nowrap overflow-hidden text-ellipsis" title={analysis.resultUnit || "--"}>
                                    {/* {analysis.resultUnit || '--'} */}
                                ...
                                <div className="truncate whitespace-nowrap overflow-hidden text-ellipsis" title={analysis.deadline ? formatDate(analysis.deadline) : "--"}>
                                ...
                                <div className="flex items-center justify-center">
                                    ...
                                    >
                                        {analysis.note ? <span className="text-xl">📝</span> : <span className="text-xl text-gray-400">📋</span>}
                                    </div>
                                </div>
                            </React.Fragment>
                        ))}
            ...
            <Breadcrumb paths={[{}]} source={originalList} setCurrentList={setCurrentList} setIsFilter={setIsFilter} searchTerm={searchTerm} setSearchTerm={setSearchTerm} />
                ...
                <div className="w-full overflow-x-auto px-4 py-2">
                    <div className="flex justify-between items-center min-w-fit">
                        ...
                        <div className="flex items-center space-x-2 flex-shrink-0">
                            ...
                            >
                                <span className="font-normal">📦 Vận đơn</span>
                            </button>
                        </div>
                        ...
                        <div className="flex items-center space-x-2 flex-shrink-0">
                            <button
                                ...
                                title={showTodayDeadlines ? "Click outside the date picker to cancel" : "Chọn khoảng thời gian để lọc theo deadline"}
                            >
                                <FaCalendarDay size={18} />
                                <span className="font-normal">Hạn trả</span>
                                {showTodayDeadlines && (
                                    <div className="relative z-1000 text-black datepicker-container flex" onClick={(e) => e.stopPropagation()}>
                                        <DatePicker
                                            ref={datePickerRef}
                                            selected={startDate}
                            ...
                            >
                                <FaCalendarDay size={18} />
                                <span className="font-normal">Quá hạn</span>
                            </button>
                            {/* Đủ kết quả button */}
                            <button
                                className={`p-2 rounded-lg border-gray-400 flex items-center justify-center focus:outline-none gap-2 py-1 ${
                                    showResultFilledFilter ? "text-white bg-blue-600" : "text-black"
                                }`}
                                onClick={toggleResultFilledFilter}
                                title={showResultFilledFilter ? "Hiển thị tất cả" : "Hiển thị chỉ tiêu đủ kết quả"}
                            >
                                <FaCheck size={18} />
                                <span className="font-normal">Đủ kết quả</span>
                            </button>
                            {/* Đã kiểm tra button */}
                            <button
                                className={`p-2 rounded-lg border-gray-400 flex items-center justify-center focus:outline-none gap-2 py-1 ${
                ...
                <div className="px-4 mb-2 text-left">
                    {location.search.includes("searchTerm=") && searchTerm && (
                        <div className="text-sm text-gray-500">
                            Tìm thấy <span className="font-medium">{currentList.length}</span> tiếp nhận có từ khoá <span className="font-medium">{searchTerm}</span>
                            <button onClick={handleClearSearch} className="ml-2 text-blue-600 px-2 py-0.5 bg-background border-2 border-gray-400">
                                Hủy
                            </button>
                        </div>
                    )}
                    ...
                    {filterInfo.isFilterActive && filterInfo.startDate && filterInfo.endDate && !location.search.includes("searchTerm=") && (
                        <div className="text-sm text-gray-500 mt-1">
                            Hiển thị <span className="font-medium">{currentList.length}</span> trong số <span className="font-medium">{filterInfo.totalItems || totalItems}</span> tiếp nhận có hạn trả
                            kết quả từ <span className="font-medium">{formatDate(filterInfo.startDate)}</span> đến <span className="font-medium">{formatDate(filterInfo.endDate)}</span>
                            {totalPages > 1 && (
                                <span>
                                    {" "}
                            ...
                            <button onClick={handleResetDateFilter} className="ml-2 text-blue-600 px-2 py-0.5 bg-background border-2 border-gray-400">
                                Hủy
                            </button>
                        </div>
                    )}
                    ...
                    {statusFilter !== null && !location.search.includes("searchTerm=") && !showOverdueFilter && !filterInfo.isFilterActive && (
                        <div className="text-sm text-gray-500 mt-1">
                            Hiển thị <span className="font-medium">{currentList.length}</span> trong số <span className="font-medium">{totalItems}</span> tiếp nhận có trạng thái{" "}
                    ...
                    {showOverdueFilter && !location.search.includes("searchTerm=") && (
                        <div className="text-sm text-gray-500 mt-1">
                            Hiển thị <span className="font-medium">{currentList.length}</span> trong số <span className="font-medium">{totalItems}</span> tiếp nhận đã quá hạn
                            {totalPages > 1 && (
                                <span>
                                    {" "}
                                    - Trang {currentPage}/{totalPages}
                    ...
                    <table className="w-full text-black">
                        <thead>
                            <tr className="border-b-2">
                                ...
                                <th className="p-1 border-b text-start max-w-28 min-w-28 cursor-pointer hover:text-[#103667] underline text-blue-700" onClick={toggleDeadlineFormat}>
                                    Hạn trả KQ
                                </th>
                                    ...
                                    {showStatusDropdown && (
                                        ...
                                        >
                                            {status.map((statusName, index) => (
                                                <div
                                                    key={index}
                                                    className={`px-4 py-2 text-sm hover:bg-gray-100 cursor-pointer ${statusFilter === index ? "bg-blue-100 text-blue-700 font-medium" : ""}`}
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                    ...
                                    <React.Fragment key={`fragment-${receipt.receiptId || receipt.id}`}>
                                        ...
                                        {samplesToShow.length === 0 ? (
                                                ...
                                                <td className="p-1 text-start align-top">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <p
                                                            ...
                                                            onClick={() => handleCopyToClipboard(!isTechnician() ? receipt.client?.clientName || "--" : "[Thông tin bị ẩn]")}
                                                        >
                                                            {!isTechnician() ? receipt.client?.clientName || "--" : "[Thông tin bị ẩn]"}
                                                        </p>
                                                        {/* Order ID */}
                                                        {receipt.orderId && !isTechnician() && <p className="text-xs text-blue-600">ĐH: {receipt.orderId}</p>}
                                                        {/* Receipt Date and Created By */}
                                                        <p className="text-xs text-gray-500">
                                                            {receipt.receiptDate && formatDate(receipt.receiptDate)} {receipt?.createdBy?.identityName || "--"}
                                                        </p>
                                                    </div>{" "}
                                                </td>
                                            ...
                                            samplesToShow.map((sample, sampleIndex) => {
                                                ...
                                                const completedTests = sample?.analyses?.filter((analysis) => analysis?.resultValue !== null && analysis?.resultValue !== "<p></p>")?.length || 0;
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        {sampleIndex === 0 && (
                                                            <>
                                                                <td
                                                                    className={`p-1 text-start align-top ${hoveredReceiptId === receipt?.receiptId ? "bg-gray-50" : ""}`}
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="flex flex-col">
                                                                        <p className="text-sm cursor-pointer" onClick={() => handleCopyToClipboard(receipt.client?.clientName || "--")}>
                                                                            {!isTechnician() ? receipt.client?.clientName || "--" : "[Thông tin bị ẩn]"}
                                                                        </p>
                                                                        {/* Order ID */}
                                                                        {receipt.orderId && !isTechnician() && <p className="text-xs text-blue-600">ĐH: {receipt.orderId}</p>}
                                                                ...
                                                                <td
                                                                    className={`p-1 text-start cursor-pointer align-top ${hoveredReceiptId === receipt.receiptId ? "bg-gray-50" : ""}`}
                                                                    rowSpan={samplesToShow.length}
                                                                ...
                                                                {showPostalColumns && (
                                                                    <>
                                                                        <td
                                                                            ...
                                                                            rowSpan={samplesToShow.length}
                                                                        >
                                                                            <div className="text-sm">{receipt._deprecated_postalOrderCreatedBy?.identityName || "--"}</div>
                                                                        </td>
                                                                        ...
                                                                        >
                                                                            <div className="text-sm">
                                                                                {receipt._deprecated_postalOrderCreatedAt ? formatDate(receipt._deprecated_postalOrderCreatedAt) : "--"}
                                                                            </div>
                                                                        </td>
                                                        ...
                                                        >
                                                            {editingField.receiptId === receipt.id && editingField.sampleId === sample.id && editingField.field === "sampleVolume" ? (
                                                                <textarea
                                                                    value={sample.sampleVolume || ""}
                                                                    onChange={(e) => handleInputChange(e, receipt.id, sample.id, "sampleVolume")}
                                                                    onBlur={(e) => handleSampleChange(receipt.id, sample.id, "sampleVolume", e.target.value)}
                                                                    className="p-1 border rounded-md w-full text-sm bg-white"
                                                                    autoFocus
                                                                />
                                                            ) : (
                                                                <div className="text-sm">
                                                                    {sample.sampleVolume ? <span className="font-medium">{sample.sampleVolume}</span> : <span className="text-gray-500">--</span>}
                                                                </div>
                                                            )}
                                                        </td>

                                                        {/* Mục đích column */}
                                                        <td className="p-1 text-start align-top cursor-pointer hover:bg-gray-100" onClick={() => handleFieldClick(receipt.id, sample.id, "purpose")}>
                                                            {editingField.receiptId === receipt.id && editingField.sampleId === sample.id && editingField.field === "purpose" ? (
                                                                <select
                                                        ...
                                                        >
                                                            {editingField.receiptId === receipt.id && editingField.sampleId === sample.id && editingField.field === "status" ? (
                                                                <select
                                                        ...
                                                        <td className="p-1 text-start align-top hover:bg-gray-100">
                                                            ...
                                                            <div className="text-sm" onMouseEnter={(e) => handleAnalysisSummaryEnter(e, sample)} onMouseLeave={handleAnalysisSummaryLeave}>
                                                                {totalTests > 0 ? (
                                                                    <span
                ...
                <div className="flex justify-between items-center mt-4">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 px-2">
                            ...
                            <select value={receiptsPerPage} onChange={(e) => handleItemsPerPageChange(parseInt(e.target.value))} className="px-3 py-2 border rounded bg-white">
                    ...
                    {(() => {
                        ...
                        return (
                            <div className="bg-white rounded-lg shadow-lg w-fit mx-4 overflow-y-auto relative">
                                    ...
                                    <h2 className="text-xl font-semibold text-gray-800">
                                        {selectedReceipt?.mode === "new"
                                            ...
                                            ? `Thông tin vận đơn ${selectedReceipt._deprecated_trackingNumber}${
                                                  selectedReceipt._deprecated_postalOrderCreatedAt ? ` - Ngày ${formatDate(selectedReceipt._deprecated_postalOrderCreatedAt)}` : ""
                                              }`
                                            : "Tạo vận đơn"}
                                    </h2>
                                    <button
                                ...
                                <div className={`${isDirectPickup ? "max-w-[50vw] min-w-[360px]" : "max-w-[80vw]"} max-h-[90vh] overflow-y-auto p-0`}>
                                    <ShipmentForm
                                        ...
                                        onOrderUpdate={(updatedReceipt) => {
                                            ...
                                            setCurrentList((prevList) => prevList.map((receipt) => (receipt.id === updatedReceipt.id ? updatedReceipt : receipt)));
