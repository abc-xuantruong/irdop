...
import { FaTrashAlt, FaCopy, FaUserCog, FaSave, FaTimes, FaRegTimesCircle, FaDatabase, FaStar, FaRegCopy, FaCheck, FaSync, FaLayerGroup, FaSearch } from "react-icons/fa";
    ...
    return (
        <tr ref={setNodeRef} style={style} className="cursor-move">
            ...
            <td className="p-1  text-center">
                <div {...attributes} {...listeners} className="h-5 box-border cursor-move inline-block p-1 hover:bg-gray-100 rounded">
...
const SampleInfor = () => {
    ...
    const openPPTWindow = () => {
        ...
        if (sample?.report?.length > 0) {
            ...
            const latestReport = sample.report.reduce((prev, current) => (new Date(prev.publish_date) > new Date(current.publish_date) ? prev : current));
    ...
    const searchParameters = async (query, page = 1) => {
        try {
            ...
            const response = await apiPost("https://red.irdop.org/v1/parameter/get", {
                columns: ["id", "parameterName", "matrix", "scientificField", "displayStyle", "fee", "accreditation", "protocolSource", "protocolCode", "technicianAlias"],
                page: page,
                itemsPerPage: 20,
                sortBy: "ASC",
    ...
    const fetchSampleFullAndAddAll = async (sampleUid) => {
        try {
            ...
            if (response.data && response.data.analysis) {
                ...
                if (duplicateCount > 0) {
                    showToast(`Đã thêm ${analyses.length} chỉ tiêu từ mẫu ${sampleUid} (${duplicateCount} chỉ tiêu đã tồn tại)`, "warning");
                } else {
                ...
                if (parameter.isDuplicate) {
                    ...
                    const analysisData = {
                        ...
                        deadline: parameter.deadline
                            ...
                            : adjustDateForApiSubmission(new Date(Date.now() + (parameter?.tatExpected?.days * 24 * 60 * 60 * 1000 || 0))),
    ...
    const updateAnalysis = async (analysis) => {
        try {
            ...
            if (analysis.resultValue !== undefined && analysis.resultValue !== "") changedFields.resultValue = convertValueToHTML(analysis.resultValue);
    ...
    const onUpdateAnalysis = async (analysis) => {
        try {
            ...
            if (analysis.resultValue !== undefined && analysis.resultValue !== "") fieldBeingUpdated.resultValue = convertValueToHTML(analysis.resultValue);
                    ...
                    <div className="w-full h-full relative flex flex-col justify-between overflow-auto">
                                        ...
                                        <div>
                                            ...
                                            <ul>
                                                {paginatedParameters.map((parameter, index) => (
                                                    <li key={parameter.id || index} className="p-2 border-b cursor-pointer hover:bg-gray-200" onClick={() => handleParameterSelect(parameter)}>
                                                        <div>
                                                            ...
                                                            {parameter.scientificField && <p className="text-start text-xs text-blue-600 w-full line-clamp-1">Lĩnh vực: {parameter.scientificField}</p>}
                                                        </div>
                                                    </li>
                                                ))}
                                            </ul>
                                            {parameterPagination.totalPages > 1 && <div className="flex justify-center mt-2 p-2 border-t bg-gray-50">{renderPageButtons()}</div>}
                                        </div>
                                    )}

                                    {/* Show no results message when not loading and no results */}
                                    {!isLoadingParameters && paginatedParameters.length === 0 && (
                                        <div className="p-4 text-center text-gray-500">Không tìm thấy chỉ tiêu nào với từ khóa "{searchTerm}"</div>
                                    )}
                                </div>
                            )}
                        </div>
                                                    ...
                                                    const newAnalyses = sampleAnalyses.filter(
                                                        (analysis) =>
                                                            !selectedParameters.some(
                                                                (selected) =>
                                                                    selected.tempId === analysis.tempId || (selected.parameterName === analysis.parameterName && selected.matrix === analysis.matrix),
                                                            ),
                                                    );
                                        ...
                                        <div className="grid grid-cols-1 gap-1 p-2">
                                            {sampleAnalyses.map((analysis, index) => {
                                                const isAlreadySelected = selectedParameters.some(
                                                    (selected) => selected.tempId === analysis.tempId || (selected.parameterName === analysis.parameterName && selected.matrix === analysis.matrix),
                                                );
                                                    ...
                                                    >
                                                        <div className="flex justify-between items-start">
                                                            <div className="flex-1">
                                                                <p className="text-start text-xs font-medium w-full line-clamp-1 text-gray-600">Nền mẫu: {analysis.matrix}</p>
                                                                <p className="text-start text-primary font-medium w-full line-clamp-1">{analysis.parameterName}</p>
                                                                ...
                                                                {analysis.scientificField && (
                                                                    <p className="text-start text-xs text-blue-600 w-full line-clamp-1">Lĩnh vực: {analysis.scientificField}</p>
                                                                )}
                                                            </div>
                                                            {isAlreadySelected && (
                        ...
                        <div className="mb-4 h-full flex overflow-y-auto text-sm flex-wrap content-start">
                            {selectedParameters.map((parameter, index) => (
                                <div
                                    ...
                                    className={`p-1 border rounded mb-2 flex text-start items-center w-fit h-fit mr-1 max-w-68 ${parameter.isDuplicate ? "bg-yellow-100 border-yellow-400" : ""}`}
                                >
                                    <div>
                                        ...
                                        <p className="text-start text-text-secondary w-full line-clamp-1">
                                            {parameter.protocolCode}
                                            {parameter?.accreditation && <b>{` (${parameter.accreditation["107"] ? "107" : ""}  ${parameter.accreditation["VILAS997"] ? "VILAS 997" : ""} )`}</b>}
                        ...
                        <div className="flex justify-between items-center">
                            <div className="flex gap-2 relative">
                                ...
                                <button className="copy-parameter-button bg-blue-500 text-white p-2 rounded hover:bg-blue-600" onClick={handleCopyParameterClick}>
                                    Copy chỉ tiêu
                                </button>
                                ...
                                {copySampleDropdownVisible && listSampleByReceipt && listSampleByReceipt.length > 0 && (
                                    <div className="copy-sample-dropdown-container absolute left-0 bottom-full mb-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-64 overflow-y-auto z-50 min-w-[200px]">
                                        {listSampleByReceipt
                                            ...
                                            .map((sid, index) => (
                                                <div key={index} className="p-2 hover:bg-gray-100 cursor-pointer border-b last:border-b-0" onClick={() => handleCopySampleSelect(sid)}>
                                                    <span className="text-sm font-medium">{sid}</span>
                                                </div>
                                            ))}
                                        {listSampleByReceipt.filter((sid) => sid !== sampleId).length === 0 && (
                                            <div className="p-2 text-sm text-gray-500 text-center">Không có mẫu khác trong phiếu tiếp nhận này</div>
                                        )}
                                    </div>
                                )}
                            </div>
    ...
    const renderFieldSelectionDialog = () => {
        ...
        return (
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50" onClick={() => setIsFieldSelectionVisible(false)}>
                <div className="bg-white p-6 rounded-lg w-[500px] max-w-[90%] relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <p className="text-sm text-gray-600 mb-4">Chọn các trường bạn muốn cập nhật cho các chỉ tiêu đã tồn tại hoặc tạo mới cho các chỉ tiêu mới.</p>
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        <label className="flex items-center cursor-pointer hover:bg-gray-50 p-2 rounded">
                            <input type="checkbox" checked={selectedFields.protocolCode} onChange={() => handleFieldToggle("protocolCode")} className="mr-3 w-4 h-4" />
                            <span>Mã phương pháp (Protocol Code)</span>
                        </label>

                        {/* Protocol Source */}
                        <label className="flex items-center cursor-pointer hover:bg-gray-50 p-2 rounded">
                            <input type="checkbox" checked={selectedFields.protocolSource} onChange={() => handleFieldToggle("protocolSource")} className="mr-3 w-4 h-4" />
                            <span>Nguồn phương pháp (Protocol Source)</span>
                        </label>

                        {/* Đơn vị */}
                        <label className="flex items-center cursor-pointer hover:bg-gray-50 p-2 rounded">
                            <input type="checkbox" checked={selectedFields.resultUnit} onChange={() => handleFieldToggle("resultUnit")} className="mr-3 w-4 h-4" />
                        ...
                        {/* Kiểm nghiệm viên */}
                        <label className="flex items-center cursor-pointer hover:bg-gray-50 p-2 rounded">
                            <input type="checkbox" checked={selectedFields.technicianId} onChange={() => handleFieldToggle("technicianId")} className="mr-3 w-4 h-4" />
                            <span>Kiểm nghiệm viên (Technician)</span>
                        </label>
                    </div>

                    <div className="flex justify-end gap-2">
                        <button className="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600" onClick={() => setIsFieldSelectionVisible(false)}>
                            Hủy
                        </button>
                        <button className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600" onClick={handleConfirmAddParameter}>
                            Xác nhận
                        </button>
                    </div>
                </div>
    ...
    const fetchDropdownLists = async () => {
        try {
            ...
            if (parametersResponse.data && Array.isArray(parametersResponse.data)) {
                const parameterNames = [...new Set(parametersResponse.data.map((item) => item.parameterName || item.parameter_name || "").filter(Boolean))];
        ...
        const fetchSample = async () => {
            try {
                ...
                setOriginalSampleValues({
                    ...
                    productType: mappedSample.productType || "",
                ...
                if (mappedSample.sampleInformation && mappedSample.sampleInformation.length > 0) {
                    ...
                    const receiptStartIndex = processedSampleInfo.findIndex((item) => item.fname.includes("Ngày tiếp nhận") || item.fname.includes("receipt date"));
    ...
    const handleCopySampleInfo = async (sampleUid) => {
        try {
            ...
            if (response.data) {
                ...
                // Find the index of the first object that contains 'Ngày tiếp nhận' or 'receipt date' in fname
                const receiptStartIndex = sampleInfo.findIndex((item) => item.fname && (item.fname.includes("Ngày tiếp nhận") || item.fname.includes("receipt date")));
    ...
    // Function to search parameters for selected analyses
    const handleSearchParameters = async () => {
        if (selectedAnalytes.length === 0) {
            Swal.fire({
                icon: "warning",
                title: "Cảnh báo",
                text: "Vui lòng chọn ít nhất một chỉ tiêu để tìm kiếm",
            });
            return;
        }

        setIsSearchingParameters(true);
        ...
        try {
            ...
            // Search for each analysis
            for (const analysis of selectedItems) {
                try {
                    const response = await apiPost("https://red.irdop.org/v1/param/search", {
                        paramName: analysis.parameterName,
                    });

                    if (response && Array.isArray(response.data) && response.data.length > 0) {
                        newSuggestions[analysis.id] = response.data;
                        // Auto-select first suggestion
                        newSelectedUpdates[analysis.id] = response.data[0].parameterId;
                    } else {
                        newSuggestions[analysis.id] = [];
                    }
                } catch (error) {
                    console.error(`Error searching for analysis ${analysis.id}:`, error);
                    newSuggestions[analysis.id] = [];
                }
            }

            setParameterSuggestions(newSuggestions);
            setSelectedParameterUpdates(newSelectedUpdates);
        } catch (error) {
            console.error("Error searching parameters:", error);
            Swal.fire({
                icon: "error",
                title: "Lỗi",
                text: error.message || "Đã xảy ra lỗi khi tìm kiếm chỉ tiêu",
            });
        } finally {
            setIsSearchingParameters(false);
        }
    };

    // Function to save parameter updates
    const handleSaveParameterUpdates = async () => {
        const analysesToUpdate = Object.keys(selectedParameterUpdates).filter((analysisId) => selectedParameterUpdates[analysisId]);

        if (analysesToUpdate.length === 0) {
            Swal.fire({
                icon: "warning",
                title: "Cảnh báo",
                text: "Không có chỉ tiêu nào được chọn để cập nhật",
            });
            return;
        }
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                setSelectedParameterUpdates({});
                setSelectedAnalytes([]);
                setSelectAll(false);

                showToast(`Đã cập nhật parameterId cho ${analysesToUpdate.length} chỉ tiêu`);
            } else {
                Swal.fire({
                    icon: "error",
                    title: "Lỗi",
                    text: response.data?.message || "Có lỗi xảy ra khi cập nhật parameterId",
                });
            }
        } catch (error) {
            console.error("Error saving parameter updates:", error);
            Swal.fire({
                icon: "error",
                title: "Lỗi",
                text: error.message || "Đã xảy ra lỗi khi cập nhật parameterId",
            });
        }
    };
            ...
            <div className="border py-2 mt-1 rounded-lg">
                ...
                <div className="flex flex-col md:flex-row md:overflow-auto">
                    ...
                    <div className="w-full border-b md:border-b-0 md:border-r pb-2 ">
                        <div className="flex justify-between items-center px-4 mb-2">
                            ...
                            <button className="bg-white text-sky-500 rounded-full p-1" onClick={handleAddCustomerField} title="Thêm thông tin khách hàng">
                        ...
                        <div className="w-full overflow-hidden hover:overflow-auto md:pb-2 lg:pb-0 pb-0 hover:pb-0 mb-1">
                            ...
                            {customerInfo?.length === 0 && <div className="text-center text-gray-500 italic py-2">Chưa có thông tin khách hàng. Nhấn nút + để thêm thông tin.</div>}
                        </div>
                    </div>
                    ...
                    <div className="w-full pt-2 md:pt-0">
                        <div className="flex justify-between items-center px-4 mb-2">
                            ...
                            <button className="bg-white text-sky-500 rounded-full p-1" onClick={handleAddReceiptField} title="Thêm thông tin tiếp nhận">
                                <AiOutlinePlus size={18} />
                            </button>
                        </div>
                        <div className="w-full overflow-hidden hover:overflow-auto md:pb-2 lg:pb-0 pb-0 hover:pb-0 mb-1">
                            ...
                            {receiptInfo?.length === 0 && <div className="text-center text-gray-500 italic py-2">Chưa có thông tin tiếp nhận. Nhấn nút + để thêm thông tin.</div>}
                        </div>
                    </div>
                </div>
                ...
                {isReportChanged && (
                    <div className="flex justify-between px-4 mt-4 pt-2 border-t">
                        ...
                        <button
                            className={`text-white text-sm rounded-lg p-1 focus:outline-none w-40 ${isReportChanged ? "bg-green-500 cursor-pointer" : "bg-gray-500 cursor-default"}`}
    ...
    const handleClickOutside = (event) => {
        ...
        if (event.target.closest(".dropdown-button") || event.target.closest(".dropdown-item") || event.target.closest(".react-datepicker") || event.target.closest(".technician-dropdown")) {
            return;
        }
    ...
    const handleUpdateNote = async () => {
        ...
        try {
            ...
            if (response?.status < 300) {
                ...
                // Update local data
                const updatedAnalytes = listAnalytes.map((item) => (item.id === selectedAnalysisForNote.id ? { ...item, note: newNote } : item));
                setListAnalytes(updatedAnalytes);
    ...
    const handleReviewAnalyses = async () => {
        ...
        try {
            ...
            const selectedItems = listAnalytes.filter((item) => selectedAnalytes.includes(item.id));
            const allReviewed = selectedItems.every((item) => item.reviewedById && String(item.reviewedById).trim() !== "");

            // Determine action: If all are reviewed, we un-review (set to null). Otherwise, we review (set to current user).
            const newStatus = allReviewed ? null : currentUser.identity_uid;

            // Create array of objects with id and reviewedById fields
            ...
            const response = await apiPost("https://red.irdop.org/v1/analysis/update", {
                analyses: analysesToConfirm.map((item) => ({
                    ...item,
                    modifiedByUid: currentUser.identity_uid,
                })),
            });
            ...
            if (response.status === 200) {
                ...
                if (allReviewed) {
                    showToast(`Đã bỏ duyệt ${selectedAnalytes.length} chỉ tiêu`, "success");
                } else {
        ...
        try {
            ...
            if (selectedAnalytes.length > 0) {
                ...
                analysesToUpdate = listAnalytes.filter(
                    (analysis) =>
                        ((!analysis.parameterUid && !analysis.parameterUid) || analysis.parameterUid === "" || analysis.parameterUid === "") &&
                        analysis.matrix &&
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    {!sampleId && <p className="mt-2 text-red-500">Không tìm thấy sampleId trong URL. Vui lòng kiểm tra đường dẫn.</p>}
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full relative">
            ...
            <div className="flex justify-end mb-1">
                <button className="text-primary border-gray-300 bg-background text-sm rounded-lg p-1 w-fit self-start active:bg-sky-100 focus:outline-none mr-2" onClick={openPPTWindow}>
                    <div className="flex items-center justify-between">
                        {"PPT"} <GrDocumentText size={15} className="ml-1.5" />
                    </div>
                </button>
                <button className="text-primary border-gray-300 bg-background text-sm rounded-lg p-1 w-fit self-start active:bg-red-100 focus:outline-none " onClick={handleDeleteSample}>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td className="w-full text-start p-1 md:min-w-80 min-m-w-40 relative">
                                            <input type="text" value={sample?.sampleId || ""} className="w-full bg-white border rounded p-1" disabled />
                                            <CopyButton textToCopy={sample?.sampleId || ""} className="absolute right-2 top-1/2 transform -translate-y-1/2" />
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                createPortal(
                                                    <div
                                                        ...
                                                        style={{
                                                            ...
                                                            top: document.getElementById("sample-matrix-input").getBoundingClientRect().bottom + window.scrollY,
                                                            left: document.getElementById("sample-matrix-input").getBoundingClientRect().left + window.scrollX,
                                                        }}
                                                    >
                                    ...
                                    <tr>
                                        ...
                                        <td className="w-full text-start p-1 flex">
                                            <textarea
                                                ...
                                                onBlur={(e) => handleFieldBlur("sampleDescription", e.target.value, originalSampleValues.sampleDescription)}
                                            />
                                        </td>
                                    </tr>
                                </tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td className="w-full text-start p-1 md:min-w-80 min-w-40">
                                            <input
                                                ...
                                                onBlur={(e) => handleFieldBlur("sampleVolume", e.target.value, originalSampleValues.sampleVolume)}
                                            />
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td className="w-full text-start p-1">
                                            <select value={sample?.purpose || ""} className="w-full bg-white border rounded p-1 editable-field" onChange={handlePurposeChange}>
                                    ...
                                    <tr>
                                        ...
                                        <td className="w-full text-start p-1 md:max-w-80 max-w-60">
                                            <select
                                                ...
                                                onBlur={(e) => handleFieldBlur("productType", e.target.value, originalSampleValues.productType)}
                                            >
                                                <option value="">-- Chọn phân loại --</option>
                                                <option value="Mẫu có giá trị cao">Mẫu có giá trị cao</option>
                                                <option value="Mẫu có tính nguy hiểm">Mẫu có tính nguy hiểm</option>
                                            </select>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td className="w-full text-start p-1 flex">
                                            <textarea
                                                ...
                                                onBlur={(e) => handleFieldBlur("additionalRequest", e.target.value, originalSampleValues.additionalRequest)}
                                            />
                                        </td>
                                    </tr>
                                </tbody>
                                    ...
                                    <div className="absolute right-0 mt-1 bg-white border rounded shadow-lg z-10 min-w-40 sample-dropdown-container">
                                        ...
                                        <ul>
                                            {listSampleByReceipt.map((sampleId) => (
                                                <li
                                                    key={sampleId}
                                                    className={`p-2 cursor-pointer hover:bg-gray-100 ${sampleId === sampleId ? "bg-gray-200" : ""}`}
                            ...
                            >
                                {isSearchingParameters ? (
                                    ...
                                    <>
                                        ...
                                        Tìm {selectedAnalytes.length > 0 ? selectedAnalytes.length : "0"}
                                    </>
                                )}
                            </button>
                            {/* Save Parameter Updates button - only show when there are suggestions */}
                            {Object.keys(parameterSuggestions).length > 0 && (
                                <button
                                    className="text-white text-sm rounded-lg px-2 py-1 flex-shrink-0 flex items-center bg-teal-500 mr-2"
                                    onClick={handleSaveParameterUpdates}
                                    title="Lưu cập nhật parameterId"
                                >
                    ...
                    <table className="text-black w-full border-2 analytes-table">
                        <thead>
                            <tr className="border-y-2">
                                <th className="p-2 border-x w-[10%] min-w-32 text-left">Kiểm duyệt</th>
                                ...
                                <th className="p-2 border-x w-[10%] min-w-48 text-left">Hiển thị trên phiếu</th>
                                ...
                                <th className="py-2 border-x w-10 min-w-10 cursor-pointer" onClick={handleSelectAll}>
                                    <input type="checkbox" checked={selectAll} onChange={handleSelectAll} className="w-4 h-4 pointer-events-none" />
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            {isAddingNewParameter && (
                                <tr className="border bg-blue-50">
                                    <td className="p-1 border relative">
                                        {/* Kiểm duyệt - không nhập khi tạo mới */}
                                    ...
                                    <td className="p-1 border relative">
                                        {/* Hiển thị trên phiếu - không nhập */}
                                        <div className="p-1 text-gray-400 italic text-center">--</div>
                                    </td>
                                    <td className="p-1 border relative">
                                    ...
                                    <td className="pt-[5px] pb-0 border align-top text-center">
                                        <div className="flex flex-col gap-0.5 items-center">
                                            <button onClick={handleSaveNewParameter} className="border-2 rounded-md p-0.5 text-xs w-fit text-primary" title="Lưu chỉ tiêu">
                                                <FaSave size={15} />
                                            </button>
                                            <button onClick={handleCancelNewParameter} className="border-2 rounded-md p-0.5 text-xs w-fit text-gray-500" title="Hủy">
                                                <FaTimes size={15} />
                                            </button>
                                        </div>
                                    </td>
                            ...
                            {listAnalytes?.map((order) => (
                                <tr key={order.id} className="border">
                                    {/* Gợi ý (Params) Column */}
                                    <td
                                        className="p-1 border relative align-top text-center"
                                        onDoubleClick={async () => {
                                            if (!isAdmin()) return;

                                            try {
                                                const newStatus = order.reviewedById && String(order.reviewedById).trim() !== "" ? null : currentUser.identity_uid;
                                                ...
                                                if (response.status === 200) {
                                                    ...
                                                    setListAnalytes(newAnalytesList);
                                                    showToast(newStatus ? "Đã duyệt chỉ tiêu" : "Đã hủy duyệt chỉ tiêu", "success");
                                                }
                                            } catch (error) {
                                                ...
                                                Swal.fire({
                                                    icon: "error",
                                                    title: "Lỗi",
                                                    text: error.message || "Đã xảy ra lỗi khi cập nhật trạng thái duyệt",
                                                });
                                            }
                                        }}
                                        style={{ cursor: isAdmin() ? "pointer" : "default" }}
                                        title={isAdmin() ? "Double-click để duyệt/hủy duyệt" : ""}
                                    >
                                        <div className="relative w-full h-full flex items-center justify-center min-h-[24px]">
                                            <span>{order.reviewedBy?.identityName || "--"}</span>
                                            {order.reviewedById && (
                                    ...
                                    <td className="p-1 border relative align-top">
                                        <div className="relative w-full">
                                            {order.accreditation && (
                                                <div className="absolute right-0.5 top-0.5 z-10 bg-cyan-600 text-white text-xs px-1 rounded cursor-default" title="Accreditation">
                                                    {order.accreditation}
                                                </div>
                                            )}
                                            {editingParameterField === order.id ? (
                                                ...
                                                <div>
                                                    ...
                                                    >
                                                        {order.parameterId && <span className="text-blue-600 font-semibold">{order.parameterId} </span>}
                                                        <span>{order.parameterName && order.parameterName.trim() !== "" ? order.parameterName : "--"}</span>
                                                    </div>

                                                    {/* Show dropdown if there are suggestions for this analysis */}
                                                    {parameterSuggestions[order.id] && parameterSuggestions[order.id].length > 0 && (
                                                        <div className="mt-1 text-xs">
                                                            <select
                                                                className="w-full border border-indigo-300 rounded px-1 py-0.5 bg-indigo-50 text-xs"
                                                                value={selectedParameterUpdates[order.id] || ""}
                                                                onChange={(e) => {
                                                                    setSelectedParameterUpdates((prev) => ({
                                                                        ...prev,
                                                                        [order.id]: e.target.value,
                                                                    }));
                                                                }}
                                                            >
                                                                {parameterSuggestions[order.id].map((suggestion) => (
                                                                    <option key={suggestion.parameterId} value={suggestion.parameterId}>
                                        ...
                                        <div className="flex flex-col">
                                            <div className="flex items-center gap-0.5">
                                                ...
                                                {editingProtocolField === order.id ? (
                                                    ...
                                                    >
                                                        <span>{order.protocolCode && order.protocolCode.trim() !== "" ? order.protocolCode : "--"}</span>
                                                    </div>
                                                )}
                                            </div>
                                    ...
                                    <td className="p-1 border relative align-top" onClick={() => handleResultValueClick(order)}>
                                        <div className="hover:border-purple-500 hover:border rounded text-center">
                                            {editingField === `result_value-${order.id}` && isEditorVisible ? (
                                                order && <TinyMceInput value={inputValue || ""} onUpdate={handleSaveContent} onKey={handleKeyDown} />
                                            ) : (
                                                <div
                                    ...
                                    <td className="p-1 border relative align-top">
                                        {editingField === `result_unit-${order.id}` ? (
                                                    ...
                                                    createPortal(
                                                        <div
                                                            ...
                                                            style={{
                                                                ...
                                                                top: document.getElementById(`result-unit-${order.id}`)?.getBoundingClientRect().bottom + window.scrollY,
                                                                left: document.getElementById(`result-unit-${order.id}`)?.getBoundingClientRect().left + window.scrollX,
                                                            }}
                                                        >
                                            ...
                                            <div className="hover:border-purple-500 hover:border rounded" onClick={() => handleResultUnitClick(order)}>
                                                <div
                                                    dangerouslySetInnerHTML={{
                                    ...
                                    <td className="p-1 border relative align-top">
                                        <div className="flex flex-col gap-1">
                                            <input
                                                type="text"
                                                className="w-full bg-white border rounded p-1 text-left text-sm"
                                                placeholder="Default"
                                                value={order.displayStyle?.default || ""}
                                                onChange={(e) => {
                                                    const updatedAnalytes = listAnalytes.map((item) =>
                                                        item.id === order.id
                                                            ? {
                                                ...
                                                onBlur={async (e) => {
                                                    try {
                                                        await apiPost("https://red.irdop.org/v1/analysis/update", {
                                                            analysis: {
                                                                id: order.id,
                                                                displayStyle: {
                                                                    default: e.target.value,
                                                                    eng: order.displayStyle?.eng || "",
                                                                },
                                                                modifiedByUid: currentUser.identity_uid,
                                                            },
                                                        });
                                                        ...
                                                        console.error("Error updating displayStyle:", error);
                                                    }
                                                }}
                                            />
                                            <input
                                                    ...
                                                    try {
                                                        await apiPost("https://red.irdop.org/v1/analysis/update", {
                                                            analysis: {
                                                                id: order.id,
                                                                displayStyle: {
                                                                    default: order.displayStyle?.default || "",
                                                                    eng: e.target.value,
                                                                },
                                                                modifiedByUid: currentUser.identity_uid,
                                                            },
                                    ...
                                    <td className="p-1 border relative align-top">
                                        <div className="relative">
                                            <button
                                                ...
                                                title={order.technician?.identityName ? `Người thực hiện: ${order.technician.identityName}` : "Chọn người thực hiện"}
                                            >
                                    ...
                                    <td className="p-1 border relative align-top">
                                        ...
                                        >
                                            {order.note ? <span className="text-2xl">📝</span> : <span className="text-2xl text-gray-400">📋</span>}
                                        </div>
                                    </td>
                                    <td className="pt-[5px] pb-0 border align-top text-center cursor-pointer" onClick={() => handleAnalyteSelect(order.id)}>
                                        <input
                                            type="checkbox"
