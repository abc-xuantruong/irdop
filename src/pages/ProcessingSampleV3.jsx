...
const ProcessingSample = () => {
	...
	const previousFiltersRef = useRef(null); // Track previous filter state to detect when filters are cleared
	...
	const fetchReceiptData = async () => {
		...
		try {
			...
			// If the contractor parameter exists, filter for EX protocol_source
				...
				// Default API call without filters - reset all filter states
				setFilterUrgent(false);
				setFilterNoResults(false);
				setFilterOverdue(false);
				setFilterContractor(false);
	...
	useEffect(() => {
		const searchParams = new URLSearchParams(location.search);
		setFilterUrgent(searchParams.has('urgent'));
		setFilterNoResults(searchParams.has('no_results'));
		setFilterOverdue(searchParams.has('overdue'));
		setFilterContractor(searchParams.has('contractor'));
	}, [location.search]);
	...
	useEffect(() => {
		...
		const hasFilterParams =
			...
			searchParams.has('no_results') ||
			searchParams.has('overdue') ||
			searchParams.has('contractor') ||
			(searchParams.has('deadline_start') && searchParams.has('deadline_end'));

		// Check if filters were cleared (had filters before, now don't have any)
		const filtersWereCleared = previousFiltersRef.current === true && !hasFilterParams;

		// Update the ref for next time
		previousFiltersRef.current = hasFilterParams;
		...
		if (queryMode === 'chitieu') {
			...
			// Fetch data when: switching modes, initial load, filter changes, or clearing filters
			if (previousMode !== 'file' || !isFetch || !processingSample || hasFilterParams || filtersWereCleared) {
				if (!isApiCallInProgress) {
					fetchReceiptData();
				}
			}
			...
			// Fetch data when: switching modes, initial load, filter changes, or clearing filters
			if (previousMode !== 'sample' || !isFetch || !processingSample || hasFilterParams || filtersWereCleared) {
				if (!isApiCallInProgress) {
					fetchReceiptData();
				}
			}
	...
	const toggleOverdueFilter = () => {
		...
		if (currentMode === 'chitieu') {
			// For chitieu mode, just update URL - iframe will handle the filtering
			const hasOverdueParam = searchParams.has('overdue');
			if (!hasOverdueParam) {
				searchParams.set('overdue', 'true');
				toast.info('Đã bật bộ lọc quá hạn');
			} else {
				searchParams.delete('overdue');
				toast.info('Đã tắt bộ lọc quá hạn');
			}
		...
		// For bangiao and file modes, continue with existing logic
		// Check if overdue filter is already active in the URL
		const hasOverdueParam = searchParams.has('overdue');
		...
		<div className="w-full h-full relative">
			...
			<div className="w-full h-full flex justify-between items-center rounded-lg mb-2">
				<div className="flex gap-2">
					...
					<button
						onClick={() => {
							// Navigate to fullscreen analysis page with current query params
							const currentParams = new URLSearchParams(location.search);
							navigate(`/processing/analysis?${currentParams.toString()}`);
						}}
						className="px-4 py-0.5 rounded-lg transition-colors flex items-center gap-2 bg-purple-500 text-white hover:bg-purple-600"
						title="Mở chỉ tiêu trong chế độ toàn màn hình"
					>
						<FaExpand size={12} />
						Toàn màn hình
					</button>
						...
						<div className="w-full flex flex-col mb-4 gap-2">
							...
							<div className="flex flex-wrap justify-between items-center gap-2 mt-2">
								...
								<div className="flex flex-wrap gap-2">
									...
									<button
										className={`px-3 py-1 text-sm rounded-lg border ${
											new URLSearchParams(location.search).has('overdue')
												? 'bg-teritary border-primary'
												: 'bg-gray-100 border-gray-300'
										}`}
									...
									<button
										className={`px-3 py-1 text-sm rounded-lg border ${
											new URLSearchParams(location.search).has('contractor')
												? 'bg-teritary border-primary'
												: 'bg-gray-100 border-gray-300'
										}`}
