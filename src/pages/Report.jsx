...
import { measureSectionsInDOM, parseContentSections, applyClientSidePagination, generatePreviewHTML, openPreviewWindow } from "../contexts/reportPreviewHelpers";
    ...
    const loadInvoiceFile = async (receiptDataParam) => {
        try {
            ...
            if (response.status === 200 && Array.isArray(response.data)) {
                ...
                const invoice = response.data.find((file) => file.userTags && Array.isArray(file.userTags) && file.userTags.some((tag) => tag.includes("Hóa đơn")));
...
<div id="customer-section" style="padding-top: 0; display: flex; flex-direction: column; border: 1px solid #000000; margin:0;">
    ...
    const generateAnalysisSection = (data) => {
        ...
        if (analysisItems.length > 0) {
            analysisRows = analysisItems
                .map((item, index) => {
                    ...
                    if (item.display_style && typeof item.display_style === "object" && !Array.isArray(item.display_style) && item.display_style.default) {
        ...
        const knParagraph = showKN ? `<p style="font-weight: bold; text-align: left; font-size: 12px; margin: 0 0 8px 0; padding: 0;">Kết quả thử nghiệm:</p>` : "";
...
<div id="footer-section" style="border-top: 1px solid #4CB748; height: 50px; display: flex; padding-top: 0pt; align-items: center;">
                        ...
                        if (contentHeaderCells[index]) {
                            ...
                            if (computedWidth > 0) {
                                ...
                                if (!currentStyle.includes("width:") && !currentStyle.includes("width :")) {
                                    contentHeaderCells[index].setAttribute("style", `${currentStyle}${currentStyle ? " " : ""}width: ${computedWidth}px;`);
                                }
                            }
                        }
                    });
    ...
    const handleSinglePreview = async () => {
        try {
            ...
            const finalHTML = generatePreviewHTML(paginatedPages, measurementData, headerHTML, footerHTML, currentRefNumber, currentSampleId);
...
<body>
            ...
            if (viewMode === "single") {
                ...
                if (response.status === 200 && response.data) {
                    ...
                    if (sampleData && savedReport.refNumber) {
                        ...
                        const existingReportIndex = updatedSampleData.reports.findIndex((r) => r.refNumber === savedReport.refNumber);
            ...
            if (viewMode === "single") {
                ...
                if (sampleData?.reports && sampleData.reports.length > 0) {
                    ...
                    if (response.status === 200 && response.data) {
                        ...
                        const spacing = '<div style="height: 15px;"></div>';
                        const contentParts = [];
                        if (reportData.customerSection) contentParts.push(ensureSectionId(reportData.customerSection, "customer-section"));
                        if (reportData.sampleSection) contentParts.push(ensureSectionId(reportData.sampleSection, "sample-section"));
                        if (reportData.analysisSection) contentParts.push(ensureSectionId(reportData.analysisSection, "analysis-section"));
                        if (reportData.commentSection) contentParts.push(ensureSectionId(reportData.commentSection, "comment-section"));
                        if (reportData.noteSection) contentParts.push(ensureSectionId(reportData.noteSection, "notes-section"));
                        if (reportData.signatureSection) contentParts.push(ensureSectionId(reportData.signatureSection, "signature-section"));
    ...
    const handleViewModeChange = async (mode) => {
        ...
        if (mode === "all") {
            ...
            const receiptId = receiptIdFromUrl || sampleId?.split("-")[0].replace("SP", "TNM") || selectedSampleId?.split("-")[0].replace("SP", "TNM");
            if (receiptId) {
    ...
    const handleSampleReportChange = async (sampleIndex, reportIndex) => {
        ...
        try {
            ...
            if (response.status === 200 && response.data) {
                ...
                const contentParts = [];
                if (reportData.customerSection) contentParts.push(ensureSectionId(reportData.customerSection, "customer-section"), spacing);
                if (reportData.sampleSection) contentParts.push(ensureSectionId(reportData.sampleSection, "sample-section"), spacing);
                if (reportData.analysisSection) contentParts.push(ensureSectionId(reportData.analysisSection, "analysis-section"), spacing);
                if (reportData.commentSection && reportData.isComment) contentParts.push(ensureSectionId(reportData.commentSection, "comment-section"), spacing);
                if (reportData.noteSection) contentParts.push(ensureSectionId(reportData.noteSection, "notes-section"), spacing);
                if (reportData.signatureSection) contentParts.push(ensureSectionId(reportData.signatureSection, "signature-section"));
    ...
    const handleSampleToggleChange = (sampleIndex, toggleName, value) => {
        ...
        if (toggleName === "showVlas" || toggleName === "showKN") {
            const sample = updatedSamples[sampleIndex];
            const newHeaderContent = generateHeaderForSample(toggleName === "showVlas" ? value : sample.showVlas, sample.currentRefNumber, toggleName === "showKN" ? value : sample.showKN);
        ...
        setTimeout(() => {
			...
			<th style="border: 1px solid black; padding: 4px 8px; background-color: #f2f2f2; font-weight: 500; width: 15%; text-align:left; font-size:12px; vertical-align: middle; line-height: 1.2; box-sizing: border-box;">
                ...
                const knParagraph = sample.showKN ? `<p style="font-weight: bold; text-align: left; font-size: 12px; margin: 0 0 8px 0; padding: 0;">Kết quả thử nghiệm:</p>` : "";
                ...
                {viewMode === "single" ? (
                    ...
                    <div className="space-y-6">
                        {allSamplesData.map((sample, index) => {
                            ...
                            return (
                                <div key={sample.sampleId} className="bg-white shadow-lg" style={{ padding: "37.8px", boxSizing: "border-box" }}>
                                    ...
                                    <div style={{ maxWidth: "720px", margin: "0 auto", marginBottom: "20px" }}>
                                        <TinyMCEEditor
                                            value={sample.headerContent || generateHeaderForSample(sample.showVlas, sample.currentRefNumber, sample.showKN)}
            ...
            <div className="flex-1 bg-white shadow-lg p-6" style={{ maxWidth: "1000px" }}>
                ...
                <div className="mb-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => setIsFileFormVisible(true)} className="px-4 py-2 rounded-lg transition bg-gray-200 text-gray-700 hover:bg-gray-300">
                            Files
                        </button>
                        <button
                ...
                {viewMode === "single" ? (
                    ...
                    <div className="mb-6 space-y-3">
                        ...
                        {allSamplesData.map((sample, index) => (
                            <div key={sample.sampleId} className={`p-3 rounded-lg space-y-2 transition-colors ${sample.isHidden ? "bg-gray-300" : "bg-gray-50"}`}>
                                <div className="flex items-center gap-4">
                                    <label className={`text-sm font-semibold whitespace-nowrap min-w-[120px] ${sample.isHidden ? "text-gray-500" : ""}`}>{sample.sampleId}:</label>
                                    <select
                ...
                <div className="flex gap-2 mb-6">
                    <button onClick={handleSave} className="px-4 py-2 flex-1 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">
                        SAVE
                    </button>

                    <button onClick={handlePreview} className="px-4 py-2 flex-1 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition">
                        PREVIEW
                    </button>

                    <button onClick={handlePublish} className="px-4 py-2 flex-1 bg-green-500 text-white rounded-lg hover:bg-green-600 transition">
                        PUBLISH
                    </button>

                    <button onClick={handleSelectLatestPublished} className="px-4 py-2 flex-1 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition text-sm">
