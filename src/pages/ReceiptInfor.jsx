...
const ReceiptInfor = ({ receipt, onSampleClick }) => {
    ...
    // Add helper function to check if user is an admin
        ...
        try {
            ...
            if (field === "sampleName" || field === "sampleDescription") {
                ...
                const updatedSampleInfo = sampleInfo.map((item) => {
                    ...
                    if (item.fname && searchKeywords.some((keyword) => item.fname.toLowerCase().includes(keyword.toLowerCase()))) {
                        found = true;
    ...
    const handleSaveContent = async (newValue) => {
        ...
        try {
            const analysis = updatedAnalytes.find((item) => item.id === parseInt(editingField.split("-")[2]) && item.sampleId.toString() === editingField.split("-")[1]);
    ...
    const handleSaveNewSample = async () => {
        ...
        const copiedSampleUid = newSample.copiedFromSampleUid;
        const hasAnalyses = copiedSampleUid && currentReceipt.samples.find((sample) => sample.sampleId === copiedSampleUid)?.analysis?.length > 0;

        if (copyCount > 1) {
            showToast(hasAnalyses ? `Đã tạo thành công ${copyCount} mẫu và sao chép chỉ tiêu!` : `Đã tạo thành công ${copyCount} mẫu!`);
        } else {
    ...
    // Add function to handle bulk review/un-review
    const handleReviewAnalyses = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                showToast(newStatus ? `Đã duyệt thành công ${selectedAnalytes.length} chỉ tiêu` : `Đã bỏ duyệt ${selectedAnalytes.length} chỉ tiêu`, "success");
            } else {
                Swal.fire({
                    icon: "error",
                    title: "Lỗi",
                    text: response.data?.message || "Có lỗi xảy ra khi cập nhật trạng thái duyệt",
                });
            }
        } catch (error) {
        ...
        try {
            ...
            if (selectedAnalytes.length > 0) {
                ...
                analysesToUpdate = listAnalytes.filter(
                    (analysis) =>
                        ((!analysis.parameterUid && !analysis.parameter_uid) || analysis.parameterUid === "" || analysis.parameter_uid === "") &&
                        analysis.matrix &&
        ...
        if (sampleToCopy) {
            ...
            try {
                ...
                if (sampleInfo) {
                    ...
                    const parsedInfo = Array.isArray(sampleInfo) ? sampleInfo : typeof sampleInfo === "string" ? JSON.parse(sampleInfo) : sampleInfo;
                    ...
                    <div className="w-1/2 pr-2">
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                className={`w-full border rounded p-1 bg-white ${checkConfirm && (newSample.sampleName || newSample.sample_name || "").trim() === "" ? "border-red-500" : ""}`}
                            />
                        </div>
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                onChange={handleNewSampleChange}
                                className={`w-full border rounded p-1 bg-white ${checkConfirm && newSample.matrix.trim() === "" ? "border-red-500" : ""}`}
                            />
                        </div>
                        <div className="mb-4">
                    ...
                    <div className="w-1/2 pl-2">
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                onChange={handleNewSampleChange}
                                className={`w-full border rounded p-1 bg-white ${checkConfirm && (newSample.sampleVolume || newSample.sample_volume || "").trim() === "" ? "border-red-500" : ""}`}
                            />
                        </div>
                        <div className="mb-4">
                            ...
                            <select
                                ...
                                onChange={handleNewSampleChange}
                                className={`w-full border rounded p-1 bg-white ${checkConfirm && newSample.purpose.trim() === "" ? "border-red-500" : ""}`}
                            >
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-medium mb-1 text-start">Phân loại sản phẩm</label>
                            <select name="productType" value={newSample.productType || ""} onChange={handleNewSampleChange} className={`w-full border rounded p-1 bg-white`}>
                ...
                <div className="mb-4 flex flex-col">
                    ...
                    {sampleInformation.map((field, index) => (
                        <div key={index} className="flex mb-2">
                            <input value={field.fname} onChange={(e) => handleAdditionalFieldChange(index, "fname", e.target.value)} className="w-1/3 min-w-36 border rounded p-1 bg-white mr-2" />
                            <input type="text" value={field.fvalue} onChange={(e) => handleAdditionalFieldChange(index, "fvalue", e.target.value)} className="w-full border rounded p-1 bg-white" />
                            <button className="text-red-500 bg-white text-sm rounded-lg p-1 px-4 focus:outline-none text-center ml-2" onClick={() => handleDeleteAdditionalField(index)}>
                                <FaTrashAlt size={20} />
                            </button>
                        </div>
                    ))}
                    <button className="bg-sky-500 text-white text-sm rounded-lg p-1 active:bg-sky-600 focus:outline-none w-32" onClick={handleAddAdditionalField}>
                        Thêm thông tin
                    </button>
                </div>{" "}
                <div className="flex justify-end mt-4">
                    <div className="flex-1 flex items-center space-x-2">
                        <select className="bg-white border rounded p-1" onChange={(e) => handleCopySample(e.target.value)} defaultValue="copy">
    ...
    const handleExcelDownload = async () => {
        ...
        if (!currentReceipt?.status || currentReceipt.status.trim() === "" || currentReceipt.status === "Chưa xác định") {
            try {
                ...
                Swal.fire({
                    icon: "warning",
                    title: "Chưa gửi email",
                    text: "Chưa gửi email tiếp nhận cho khách hàng, chưa thể tải xuống file excel bàn giao mẫu thử nội bộ",
                    timer: 3000,
                    timerProgressBar: true,
                    showConfirmButton: false,
                });
                // Just call the API without downloading
                await apiGetBlob(`https://red.irdop.org/v1/excel/handover?receiptId=${receiptId}`);
            } catch (error) {
    ...
    const handlePayStatusChange = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                showToast(`Đã cập nhật trạng thái thanh toán thành ${newPayStatus === 1 ? "đã thanh toán" : "chưa thanh toán"}!`);
            } else {
                Swal.fire({
                    icon: "error",
                    title: "Lỗi",
    ...
    const renderPayStatusConfirm = () => (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex justify-center items-center z-50">
            <div className="bg-white p-4 rounded-lg w-[400px] h-[200px] relative flex flex-col justify-between">
                ...
                <p>{currentReceipt?.pay_status === 1 ? "Bạn muốn thay đổi trạng thái thanh toán sang Chưa thanh toán?" : "Bạn muốn thay đổi trạng thái thanh toán sang Đã thanh toán?"}</p>
                <div className="flex justify-end mt-4">
    ...
    const fetchEmailFormData = async () => {
        ...
        try {
            ...
            const response = await apiGet(`https://red.irdop.org/v1/get/email/receipt_form?receiptId=${currentReceipt.receiptId}`);
    ...
    const renderPPTTable = () => {
        return <div className="p-4 text-center text-gray-500">Report functionality has been removed from this component.</div>;
    };
        ...
        <div className="w-full">
                    ...
                    {!isTechnician() && (
                        <div className="w-full flex flex-col sm:flex-row justify-end sm:justify-between items-start sm:items-center mb-1">
                            ...
                            <div className="flex flex-row items-center gap-2 w-full sm:w-auto justify-start">
                                <button className="bg-background border-gray-300 text-primary font-medium py-1 px-2 rounded-lg text-sm whitespace-nowrap max-w-[80px]" onClick={handleExcelDownload}>
                                ...
                                <button
                                    ...
                                    onClick={() => window.open(`/dashboard/receipt/print_sp?receiptId=${currentReceipt?.receiptId}`, "_blank")}
                                >
                                    <div className="flex items-center justify-center gap-1">
                                        Tag <FaTag size={16} />
                                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="flex flex-col items-start">
                            ...
                            <div className="w-full">
                                ...
                                <div className="flex justify-start items-start mb-1">
                                    ...
                                    <div className="w-2/3 px-2 py-0 text-sm text-left border border-white rounded-lg">{currentReceipt?._deprecated_trackingNumber || "--"}</div>
                                </div>
                                <div className="flex justify-start items-start mb-1">
                                            ...
                                            <div className="mb-2 w-fit h-fit">
                                                {isLoadingImage ? (
                                                    ...
                                                    <div className="border border-gray-300 rounded-lg overflow-hidden">
                                                        <img src={sampleImageUrl} alt="Hình ảnh mẫu" className="w-full h-auto max-h-48 object-contain" onError={() => setImageError(true)} />
                                                    </div>
                                                ) : null}
                                            </div>
                                        )}
                                ...
                                <div className="flex justify-start items-start mb-1">
                                    ...
                                    <div className="w-2/3">
                                        ...
                                        <div className="block md:hidden">
                                            <button onClick={handleQRScan} className="flex items-center gap-2 bg-blue-500 text-white px-3 py-2 rounded-lg hover:bg-blue-600 transition-colors">
                                                <FaCamera size={16} />
                                    ...
                                    <div className="flex justify-start items-start mb-1">
                                        ...
                                        {editingGeneralField === "client.invoiceInfo" ? (
                                            <textarea
                                                ...
                                                onKeyDown={(e) => handleFieldKeyDown(e, "client.invoiceInfo", currentReceipt?.client?.invoiceInfo)}
                                                autoFocus
                                            />
                                        ) : (
                                            <div
                                ...
                                <div className="flex justify-start items-start mb-1">
                                    ...
                                    <div className="flex items-center">
                                        ...
                                        <div className="flex items-center ml-2 cursor-pointer" onClick={handlePayStatusToggle}>
                                            <div className={`min-w-2 h-2 rounded-full mr-1 ${currentReceipt?.pay_status === 1 ? "bg-green-600" : "bg-red-500"}`}></div>
                                            <span className={`font-medium text-xs min-w-28 ${currentReceipt?.pay_status === 1 ? "text-green-600" : "text-red-500"}`}>
                        ...
                        <div className="flex flex-col items-start">
                            ...
                            <div className="w-full">
                                ...
                                <div className="flex justify-start items-start mb-1">
                                    <label className="block text-sm font-medium text-gray-700 min-w-32 text-left">Người nhận (khác)</label>
                                ...
                                <div className="flex justify-start items-start mb-1">
                                    ...
                                    {editingGeneralField === "reportRecipient.other" ? (
                                        <textarea
                                            ...
                                            onKeyDown={(e) => handleFieldKeyDown(e, "reportRecipient.other", currentReceipt?.reportRecipient?.other)}
                                            autoFocus
                                        />
                                    ) : (
                                        <div
            ...
            <div className="flex justify-between items-start sm:h-10 sm:flex-row flex-col h-[76px] mt-4">
                <div className="w-full flex justify-start overflow-auto mr-1">
                    <button className={`px-2 py-1 rounded-lg focus:outline-none h-fit min-w-40  ${viewMode === "sample" ? "bg-blue-200" : "bg-gray-200"}`} onClick={() => setViewMode("sample")}>
                        Mẫu thử
                    </button>
                    <button className={`ml-2 px-2 py-1 rounded-lg focus:outline-none h-fit min-w-40 ${viewMode === "analyte" ? "bg-blue-200" : "bg-gray-200"}`} onClick={() => setViewMode("analyte")}>
                        Chỉ tiêu
                    </button>
                </div>
            </div>
                                ...
                                <table className="text-black w-full relative z-0">
                                    <thead>
                                        <tr className="border-y-2">
                                            ...
                                            <th className="py-2 border-x w-20 min-w-20">Kiểm duyệt</th>
                                            ...
                                            <th className="py-2 border-x w-10 min-w-10 cursor-pointer" onClick={handleSelectAllAnalytes}>
                                                <input type="checkbox" checked={selectAllAnalytes} onChange={handleSelectAllAnalytes} className="w-4 h-4 pointer-events-none" />
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {listAnalytes.map((order) => (
                                            <tr key={`${getSampleUid(order.sampleId || order.sample_id)}-${order.id}`}>
                                                ...
                                                <td className="p-1 border text-start">
                                                    <div>
                                                        {/* Display current parameter name with parameterId if available */}
                                                        <div>
                                                            {order.parameterId && <span className="text-blue-600 font-semibold">{order.parameterId} </span>}
                                                            <span>{order.parameterName}</span>
                                                        </div>
                            ...
                            <table className="min-w-full text-black">
                                <thead>
                                    <tr className="">
                                        ...
                                        <th className="py-2 border-2 text-start pl-2 w-32 min-w-32">Phân loại</th>
                                ...
                                <tbody className="border-2">
                                    {currentReceipt?.samples.map((sample, sampleIndex) => {
                                        ...
                                        const completedTests = analyses.filter((order) => order?.resultValue !== null && order?.resultValue !== "<p></p>")?.length || 0;
                                            ...
                                            <tr key={sample.id}>
                                                <td className="p-2 px-1 border text-start text-text-secondary relative">
                                                    ...
                                                    <span
                                                        ...
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            window.open(`/dashboard/receipt/print_sp?receiptId=${currentReceipt?.receiptId}&sampleId=${sample.sampleId}`, "_blank");
                                                        }}
                                                    >
                                                        <FaTag size={14} />
                                                    </span>
                                                ...
                                                <td className="p-2 border text-start">
                                                    {isEditMode ? (
                                                        <textarea
                                                            ...
                                                            onKeyDown={(e) => handleTextareaKeyDown(e, sample.id, "sampleDescription", e.target.value)}
                                                ...
                                                <td className="p-2 border text-start">
                                                    {isEditMode ? (
                                                        <select
                                                            ...
                                                            onChange={(e) => {
                                                                handleSampleChange(sample.id, "productType", e.target.value);
                                                                handleSampleApiUpdate(sample.id, "productType", e.target.value);
                                                            }}
                                                            className="p-1 border rounded-md w-full text-sm bg-white"
                                                        >
                                                            <option value="">--</option>
                                                        ...
                                                        displayValue(sample.productType)
                                                    )}
                                                </td>
                                                <td className="p-2 border text-start">
                                                ...
                                                <td className="p-2 border text-start">
                                                    {isEditMode ? (
                                                        <textarea
                                                            ...
                                                            onKeyDown={(e) => handleTextareaKeyDown(e, sample.id, "additional_request", e.target.value)}
