...
async function generateSectionsFromSample(sectionsData, sample_uid) {
	try {
		...
		let sampleData = sampleResponse.data;

		// FIXED: Check if all results are complete and update test date before generating report
		let isResultFull = true;

		for (const test of sampleData.analysis || []) {
			if (test?.result_value == null || test?.result_value === '<p></p>' || test?.result_value === '') {
				isResultFull = false;
				break;
			}
		}
		...
		if (isResultFull) {
			for (const info of sampleData.sample_information || []) {
				// Check if this is a test date field
				if (info.fname && (info.fname.includes('Ngày thử nghiệm') || info.fname.includes('test date'))) {
					// Check if fvalue is empty before assigning a new date range
					if (info.fvalue === '') {
						const startDate = new Date(sampleData.handover_at || sampleData.created_at).toLocaleDateString('vi-VN');
						const endDate = new Date().toLocaleDateString('vi-VN');
						const testDate = `${startDate} - ${endDate}`;
						info.fvalue = testDate;
					}
				}
			}

			// Update the sample with the new test date
			try {
				const sample = sampleData;
				node.warn(sample);
				// const updateResponse = await table.Sample.update(sample);

				// if (updateResponse.status === 200) {
				// 	node.warn(`Successfully updated test date for sample ${sample_uid}`);
				// } else {
				// 	node.warn(`Failed to update sample ${sample_uid}: status=${updateResponse.status}`);
				// }
			} catch (updateError) {
				node.warn(`Error updating sample ${sample_uid}: ${updateError.message}`);
			}
		}
    ...
    <div class="content_page_header_box" id="thead" style="position:relative; height: fit-content;">
        ...
        <div style="padding-top:1mm; position:relative;">
            <div style="position:relative; text-align:left;">
                ...
                style="font-weight:820; font-size:21pt; color:#0058A3; height: 30px;">
...
// FIXED: Updated generateCustomerSection with font-weight: 760 for client name to match Report.jsx
...
// FIXED: Updated generateAnalysisSection with accreditation scope column to match Report.jsx
	...
	if (analysisItems.length > 0) {
		analysisRows = analysisItems
			.map((item, index) => {
				...
				let referenceCell = '';
				if (showReference) {
					if (index < referenceValues.length && referenceValues[index]) {
						referenceCell = referenceValues[index];
					} else {
 ...
 */
function getImprovedPrintCSSWithFonts(A4, includeContentPositioning = true, headerHeight = 150) {
